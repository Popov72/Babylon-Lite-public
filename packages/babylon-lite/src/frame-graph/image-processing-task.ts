...
function createImageProcessingState(engine: EngineContext, source: ImageProcessingSource): ImageProcessingState {
    ...
    const common = wgsl`struct P{e:f32,c:f32,t:f32,p:f32}
...
return vec4f(max(c,vec3f(0)),r.a);}`;
    const textureDecl = multisampled ? wgsl`@group(0)@binding(1)var s:texture_multisampled_2d<f32>;` : wgsl`@group(0)@binding(1)var s:texture_2d<f32>;`;
    const fragment = multisampled
        ? wgsl`@fragment fn fs(@builtin(position) q:vec4f)->@location(0) vec4f{let d=textureDimensions(s);let px=clamp(vec2i(q.xy),vec2i(0),vec2i(d)-1);let n=textureNumSamples(s);var c=vec4f(0);for(var i=0u;i<n;i++){c+=ip(textureLoad(s,px,i));}return c/f32(n);}`
        : wgsl`@fragment fn fs(@builtin(position) q:vec4f)->@location(0) vec4f{let d=textureDimensions(s);return ip(textureLoad(s,clamp(vec2i(q.xy),vec2i(0),vec2i(d)-1),0));}`;
    const shader = device.createShaderModule({ code: wgsl`${common}${textureDecl}${fragment}` });
