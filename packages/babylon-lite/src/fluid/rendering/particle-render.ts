...
@vertex fn vs(@builtin(vertex_index) vi: u32, @builtin(instance_index) ii: u32) -> VOut {
    ...
    o.uv = c;
    // Colour by speed from the settable base tint. Scale the authored colour instead of adding
    // white so saturated colours keep their hue while moving (cam.tint.w = 0 disables this for sand).
    let t = clamp(dbg[ii] * cam.misc.y, 0.0, 1.0);
    let base = cam.tint.rgb;
    let hot = mix(base, min(base * 1.5, vec3<f32>(1.0)), cam.tint.w);
