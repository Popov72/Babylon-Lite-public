...
type FoamSurfaceDepthMode = "unfiltered" | "screen" | "polygon";
...
interface FoamSplatPipelineCache {
    readonly splat: Record<FoamSurfaceDepthMode, GPURenderPipeline | null>;
    readonly activeSplat: Record<FoamSurfaceDepthMode, GPURenderPipeline | null>;
}

let foamSplatPipelineCaches: WeakMap<GPUDevice, FoamSplatPipelineCache> | null = null;

function getFoamSplatPipeline(device: GPUDevice, activeParticles: boolean, surfaceDepthMode: FoamSurfaceDepthMode): GPURenderPipeline {
    foamSplatPipelineCaches ??= new WeakMap();
    let cache = foamSplatPipelineCaches.get(device);
    if (!cache) {
        cache = {
            splat: { unfiltered: null, screen: null, polygon: null },
            activeSplat: { unfiltered: null, screen: null, polygon: null },
        };
        foamSplatPipelineCaches.set(device, cache);
    }
    const pipelines = activeParticles ? cache.activeSplat : cache.splat;
    let pipeline = pipelines[surfaceDepthMode];
    if (pipeline) {
        return pipeline;
    }
    const activeSuffix = activeParticles ? "-active" : "";
    const label = `fluid-foam-splat${activeSuffix}-${surfaceDepthMode}`;
    const module = device.createShaderModule({ label, code: buildSplatWgsl(activeParticles, surfaceDepthMode) });
    pipeline = device.createRenderPipeline({
        label,
        layout: "auto",
        vertex: { module, entryPoint: "vs" },
        fragment: { module, entryPoint: "fs", targets: [{ format: FOAM_ACCUM_FORMAT, blend: ADD_BLEND }] },
        primitive: { topology: "triangle-list", cullMode: "none" },
    });
    pipelines[surfaceDepthMode] = pipeline;
    return pipeline;
}
...
export function createFoamRenderTask(
    ...
    const splatPipes: Record<FoamSurfaceDepthMode, GPURenderPipeline | null> = {
        unfiltered: null,
        screen: null,
        polygon: null,
    };
    const activeSplatPipes: Record<FoamSurfaceDepthMode, GPURenderPipeline | null> = {
        unfiltered: null,
        screen: null,
        polygon: null,
    };
    ...
    function build(): void {
        if (splatPipes.unfiltered) {
            return;
        }
        for (const surfaceDepthMode of FOAM_SURFACE_DEPTH_MODES) {
            splatPipes[surfaceDepthMode] = getFoamSplatPipeline(device, false, surfaceDepthMode);
            activeSplatPipes[surfaceDepthMode] = getFoamSplatPipeline(device, true, surfaceDepthMode);
        }
    ...
    function updateSplatUniform(sim: FluidSim): void {
    ...
    function executeScreen(colorView: GPUTextureView, sim: FluidSim): number {
        const pool = sim.diffuse!;
        if (!blurPipe || !compPipe) {
            return 0;
        }
        ...
        // 1. Splat → accumulation (additive, depth-classified + occluded).
        {
            ...
            const surfaceDepthMode: FoamSurfaceDepthMode = surfaceFiltering ? (polygonSurfaceDepth ? "polygon" : "screen") : "unfiltered";
            const activePipe = (activeParticles ? activeSplatPipes : splatPipes)[surfaceDepthMode];
            if (!activePipe) {
                return 0;
            }
    ...
    return {
        ...
        execute(): number {
            ...
            const r = executeScreen(colorView, sim);
