...
export interface PbfOptions extends FluidSimBaseOptions {
     ...
     *  volume-sampled particle set (see fluid/sampling/volume), mirroring the same
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>, @builtin(num_workgroups) groups: vec3<u32>) {
    let i = linearDispatchIndex(gid, groups);
...
@compute @workgroup_size(${WORKGROUP_SIZE})
fn main(@builtin(global_invocation_id) gid: vec3<u32>, @builtin(num_workgroups) ng: vec3<u32>) {
    let k = linearDispatchIndex(gid, ng);
    if (k >= sim.live) { return; }
...
// Optional confinement using the per-demo sceneSdf: push penetrating particles back
// along the SDF gradient (position only — PBF derives velocity in finalize, so a
// moving boundary stirs for free). The unified SDF confines the interior cavity AND
// the exterior/ground alike, so drained fluid is caught by the SAME field — no
// per-particle "escaped" state.
...
// The per-demo sceneSdf is optional: when null the density correction still runs,
// just without scene collision push-out.
function buildApplyWgsl(scene: SceneSdfSpec | null): string {
...
export function createPbfSim(engine: EngineContext, options: PbfOptions = {}): FluidSim {
    ...
    const debugBuffer = device.createBuffer({ label: "fluid-debug", size: count * 4, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST | GPUBufferUsage.COPY_SRC });
    ...
    // Apply-pass variants (including the null-scene density-only path) are cached by
    // WGSL source so re-selecting an equivalent demo or disabling collisions is instant.
    const applyPipelineCache = new Map<string, GPUComputePipeline>();
    function getApplyPipeline(scene: SceneSdfSpec | null): GPUComputePipeline {
    ...
    let applyPipeline: GPUComputePipeline = getApplyPipeline(null);
    ...
    function buildApplyBG(pipeline: GPUComputePipeline, scene: SceneSdfSpec | null): GPUBindGroup {
    ...
    let applyBG: GPUBindGroup = buildApplyBG(applyPipeline, null);
    ...
    function ensureFoam(cfg: FoamConfig): void {
        if (!foamNormalsPipeline) {
            const nextFoamNormalsPipeline = computePipeline("fluid-foam-normals", FOAM_NORMALS_WGSL);
            const nextFoamDenseEmitPipeline = computePipeline("fluid-foam-emit", buildFoamEmitWgsl(false));
            const nextFoamDenseUpdatePipeline = computePipeline("fluid-foam-update", buildFoamUpdateWgsl(false));
            const [nextSortedNormalBuffer, nextFoamParamsBuffer, nextDiffuseHeadBuffer] = createGpuBuffersAtomically(device, [
                { label: "fluid-foam-sorted-normals", size: count * 16, usage: GPUBufferUsage.STORAGE },
                { label: "fluid-foam-params", size: FOAM_BYTES, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST },
                { label: "fluid-foam-head", size: 16, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST },
            ]);
            foamNormalsPipeline = nextFoamNormalsPipeline;
            foamDenseEmitPipeline = nextFoamDenseEmitPipeline;
            foamDenseUpdatePipeline = nextFoamDenseUpdatePipeline;
            sortedNormalBuffer = nextSortedNormalBuffer!;
        ...
        if (nextActiveParticles && !foamActiveUpdatePipeline) {
            const nextFoamActiveEmitPipeline = computePipeline("fluid-foam-emit-active", buildFoamEmitWgsl(true));
            const nextFoamActiveUpdatePipeline = computePipeline("fluid-foam-update-active", buildFoamUpdateWgsl(true));
            const nextFoamActivePreparePipeline = computePipeline("fluid-foam-active-prepare", FOAM_ACTIVE_PREPARE_WGSL);
            const nextFoamActiveFinishPipeline = computePipeline("fluid-foam-active-finish", FOAM_ACTIVE_FINISH_WGSL);
        ...
        const diffuseIndex = resizePool ? descriptors.push({ label: "fluid-foam-pool", size: cap * 32, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST }) - 1 : -1;
        const activeIndex = rebuildActiveResources
            ? descriptors.push(
                  { label: "fluid-foam-active-state", size: foamActiveStateBytes(cap), usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST },
                  { label: "fluid-foam-active-dispatch", size: 12, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.INDIRECT },
                  { label: "fluid-foam-draw-indirect", size: 16, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.INDIRECT | GPUBufferUsage.COPY_DST }
              ) - 3
            : -1;
    ...
    const timestepScheduler = createFluidTimestepScheduler({ maximumSubstepsPerFrame: 1 });
    return {
        count,
        ...
        step(encoder: GPUCommandEncoder, dt: number): void {
            ...
            const schedule = scheduleFluidTimestep(timestepScheduler, dt, 1, 1 / 30);
            if (!schedule) {
                return;
            }
            dt = schedule.frameDeltaSeconds;
