...
import { buildLinearDispatchIndexWgsl, createMlsFixedPointCodec, MLS_FIXED_POINT_CODEC_FINALIZATION_WGSL, MLS_FIXED_POINT_CODEC_WGSL } from "./wgsl-shared.js";
...
//   misc2 (restitution, frameDt, _, _)
//   codec (massScale, inverseMassScale, momentumScale, inverseMomentumScale)
//   codecLimits (maxResolvedSpeed, maxAffineFrobenius, maxVelocityContribution, densityRatioCap)
//   codecExtra (densityFloorRatio, _, _, _)
//   codecCounts (observed maxCellContributors, _, _, _)
...
const CODEC_BASE_F32 = MISC2_BASE_F32 + 4;
const CODEC_LIMITS_BASE_F32 = CODEC_BASE_F32 + 4;
const CODEC_EXTRA_BASE_F32 = CODEC_LIMITS_BASE_F32 + 4;
const CODEC_COUNTS_BASE_F32 = CODEC_EXTRA_BASE_F32 + 4;
const PARAMS_F32 = CODEC_COUNTS_BASE_F32 + 4;
const PARAMS_BYTES = PARAMS_F32 * 4;
const LINEAR_DISPATCH_WGSL = buildLinearDispatchIndexWgsl(WORKGROUP_SIZE);
...
struct Params {
    ...
    misc2: vec4<f32>,       // x = restitution, y = frameDt, _, _
    codec: vec4<f32>,       // mass scale/inverse, momentum scale/inverse
    codecLimits: vec4<f32>, // max speed, max affine Frobenius, max momentum contribution, density cap
    codecExtra: vec4<f32>,  // density floor, _, _, _
    codecCounts: vec4<u32>, // observed max particles per P2G base cell, _, _, _
};
...
${LINEAR_DISPATCH_WGSL}
${MLS_FIXED_POINT_CODEC_WGSL}
...
@group(0) @binding(4) var<storage, read_write> activeCount: array<atomic<u32>>;`
    ...
    const accumulationBinding = fusedBlockDiscovery ? 6 : 4;
    const cellCountBinding = fusedBlockDiscovery ? 7 : 5;
...
@group(0) @binding(${accumulationBinding}) var<storage, read_write> accumulationState: array<atomic<u32>>;
@group(0) @binding(${cellCountBinding}) var<storage, read_write> cellCount: array<atomic<u32>>;
@compute @workgroup_size(${WORKGROUP_SIZE})
fn main(@builtin(global_invocation_id) gid: vec3<u32>, @builtin(num_workgroups) ng: vec3<u32>) {
    let i = gid.x + gid.y * ng.x * ${WORKGROUP_SIZE}u;
...
${increment}
    let population = atomicAdd(&cellCount[u32(index1D(c, p))], 1u) + 1u;
    atomicMax(&accumulationState[2], population);
}`;
}

const FINALIZE_CODEC_WGSL = /* wgsl */ `
${COMMON_WGSL}
${MLS_FIXED_POINT_CODEC_FINALIZATION_WGSL}
@group(0) @binding(0) var<storage, read_write> p: Params;
@group(0) @binding(1) var<storage, read_write> accumulationState: array<atomic<u32>>;
@compute @workgroup_size(1)
fn main() {
    let observed = max(atomicLoad(&accumulationState[2]), 1u);
    atomicMax(&accumulationState[1], observed);
    if (atomicLoad(&accumulationState[0]) != 0u) {
        return;
    }
    let result = deriveMlsFixedPointCodec(observed, p.codecLimits.z);
    if (result.valid == 0u) {
        atomicStore(&accumulationState[0], 1u);
        return;
    }
    p.codec = result.codec;
    p.codecCounts = vec4<u32>(observed, 0u, 0u, 0u);
}`;
...
@group(0) @binding(2) var<storage, read> accumulationState: array<u32>;
@compute @workgroup_size(1)
fn main() {
    if (accumulationState[0] != 0u) {
        args[0] = 0u;
        args[1] = 0u;
        args[2] = 1u;
        return;
...
const DENSE_P2G_INDIRECT_WGSL = /* wgsl */ `
${COMMON_WGSL}
${BLOCK_WGSL}
const MAX_GROUPS: u32 = ${MAX_WORKGROUPS}u;
@group(0) @binding(0) var<storage, read_write> args: array<u32>;
@group(0) @binding(1) var<storage, read> accumulationState: array<u32>;
@group(0) @binding(2) var<uniform> p: Params;
@compute @workgroup_size(1)
fn main() {
    if (accumulationState[0] != 0u) {
...
${blockLookup}
    ...
    for (var k = tid; k < cnt; k = k + ${WORKGROUP_SIZE}u) {
        ...
        for (var gz = 0; gz < 3; gz++) {
            ...
            atomicAdd(&tileMass[ln], encodeMass(weight, p)); // particle mass = 1
        }}}
    }
    workgroupBarrier();
...
${blockLookup}
    ...
    for (var k = tid; k < cnt; k = k + ${WORKGROUP_SIZE}u) {
        ...
        let v = clampCodecVelocity(particles[pi].v, p);
        let C = clampCodecAffine(particles[pi].C, p);
        let base = cellOf(pos, p);
        let w = weightsOf(pos, p);
        let lbase = base - B0;
        ...
        for (var gz = 0; gz < 3; gz++) {
            ...
            density += decodeMass(cells[${massIndex}].mass, p) * weight;
        }}}
        ...
        if (density > 0.0) {
            let pressureDensity = clampPressureDensity(density, p.sim0.z, p);
            let stressDensity = clampStressDensity(density, p.sim0.z, p);
            let volume = 1.0 / stressDensity;
            let pressure = max(0.0, p.sim0.w * (pressureDensity / p.sim0.z - 1.0));
        ...
        for (var gz = 0; gz < 3; gz++) {
            ...
            let ln = ((lbase.x + gx) * i32(TN_U) + (lbase.y + gy)) * i32(TN_U) + (lbase.z + gz);
            atomicAdd(&tileVx[ln], encodeMomentum(vel.x, p));
            atomicAdd(&tileVy[ln], encodeMomentum(vel.y, p));
            atomicAdd(&tileVz[ln], encodeMomentum(vel.z, p));
        }}}
    }
    workgroupBarrier();
...
@group(0) @binding(8) var<storage, read> accumulationState: array<u32>;
${activeDecls}
${pageDecls}
...
${invocation}
    ...
    if (accumulationState[0] != 0u) { return; }
    let c = cells[i];
    if (c.w <= 0) { return; }
    let invMass = 1.0 / decodeMass(c.w, p);
    var v = vec3<f32>(decodeMomentum(c.x, p), decodeMomentum(c.y, p), decodeMomentum(c.z, p)) * invMass;
...
${wall}
    v = clampCodecVelocity(v, p);
    cells[i] = vec4<i32>(encodeMomentum(v.x, p), encodeMomentum(v.y, p), encodeMomentum(v.z, p), c.w);
}`;
}
...
// The per-demo sceneSdf is optional: when null the particles still advect, just
// without scene collision push-out. External forces run as their own dedicated
// compute pass (mpm-force) before the p2g transfer, so G2P stays force-free.
...
${PAGE_HELPERS_WGSL}`
    ...
    const confine = scene ? g2pConfineSdf(scene) : "";
    return /* wgsl */ `
${COMMON_WGSL}
${PARTICLE_STRUCT}
${WEIGHTS_WGSL}
...
@compute @workgroup_size(${WORKGROUP_SIZE})
fn main(@builtin(global_invocation_id) gid: vec3<u32>, @builtin(num_workgroups) ng: vec3<u32>) {
    let i = linearDispatchIndex(gid, ng);
    if (i >= p.counts.x) { return; }
    if (!fluidParticleIsActive(i)) { return; }
    ${pagedGrid ? "if (pageState[2] != 0u) { return; }" : ""}
    if (accumulationState[0] != 0u) { return; }
    ...
    for (var gz = 0; gz < 3; gz++) {
        ...
        let wv = vec3<f32>(decodeMomentum(cv.x, p), decodeMomentum(cv.y, p), decodeMomentum(cv.z, p)) * weight;
        vel += wv;
    ...
    var C = clampCodecAffine((B * Dinv) * p.sim1.z, p);
    vel = clampCodecVelocity(vel * p.sim1.y, p);
    let dt = p.sim0.x;
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>, @builtin(num_workgroups) ng: vec3<u32>) {
    ...
    particles[i].v = clampCodecVelocity(v, p);
}`;
}

function buildCopyWgsl(pagedGrid: boolean): string {
    const pageDecl = pagedGrid ? "@group(0) @binding(7) var<storage, read> pageState: array<u32>;" : "";
    return /* wgsl */ `
${PARTICLE_STRUCT}
${FLUID_LIFECYCLE_STRUCT_WGSL}
${FLUID_LIFECYCLE_RUNTIME_WGSL}
...
@group(0) @binding(5) var<storage, read_write> lifecycle: FluidLifecycle;
@group(0) @binding(6) var<storage, read> accumulationState: array<u32>;
${pageDecl}
${LINEAR_DISPATCH_WGSL}
@compute @workgroup_size(${WORKGROUP_SIZE})
fn main(@builtin(global_invocation_id) gid: vec3<u32>, @builtin(num_workgroups) ng: vec3<u32>) {
...
function buildCommitFrameWgsl(pagedGrid: boolean): string {
    const pageDecl = pagedGrid ? "@group(0) @binding(5) var<storage, read> pageState: array<u32>;" : "";
    return /* wgsl */ `
${PARTICLE_STRUCT}
${FLUID_LIFECYCLE_STRUCT_WGSL}
@group(0) @binding(0) var<storage, read> workingParticles: array<Particle>;
@group(0) @binding(1) var<storage, read_write> particles: array<Particle>;
@group(0) @binding(2) var<storage, read_write> workingLifecycle: FluidLifecycle;
@group(0) @binding(3) var<storage, read_write> lifecycle: FluidLifecycle;
@group(0) @binding(4) var<storage, read> accumulationState: array<u32>;
${pageDecl}
${LINEAR_DISPATCH_WGSL}
@compute @workgroup_size(${WORKGROUP_SIZE})
fn main(@builtin(global_invocation_id) gid: vec3<u32>, @builtin(num_workgroups) ng: vec3<u32>) {
    ...
    ${pagedGrid ? "if (pageState[2] != 0u) { return; }" : ""}
    let i = linearDispatchIndex(gid, ng);
    if (i >= arrayLength(&workingParticles)) { return; }
    particles[i] = workingParticles[i];
    atomicStore(&lifecycle.states[i], atomicLoad(&workingLifecycle.states[i]));
    if (i == 0u) {
        atomicStore(&lifecycle.activeCount, atomicLoad(&workingLifecycle.activeCount));
        lifecycle.capacity = workingLifecycle.capacity;
        lifecycle.reserved0 = workingLifecycle.reserved0;
        lifecycle.reserved1 = workingLifecycle.reserved1;
    }
}`;
}
...
function buildFoamActivePrepareWgsl(pagedGrid: boolean): string {
    const pageDecl = pagedGrid ? "@group(0) @binding(3) var<storage, read> pageState: array<u32>;" : "";
    return /* wgsl */ `
...
function buildFoamActiveFinishWgsl(pagedGrid: boolean): string {
    const pageDecl = pagedGrid ? "@group(0) @binding(4) var<storage, read> pageState: array<u32>;" : "";
    return /* wgsl */ `
@group(0) @binding(0) var<storage, read_write> state: array<atomic<u32>>;
...
@group(0) @binding(3) var<storage, read> accumulationState: array<u32>;
${pageDecl}
@compute @workgroup_size(1)
fn main() {
    if (accumulationState[0] != 0u) { return; }
...
${headDecl}
@group(0) @binding(${lifecycleBinding}) var<storage, read_write> lifecycle: FluidLifecycle;
@group(0) @binding(9) var<storage, read> accumulationState: array<u32>;
${activeDecl}
${pageDecls}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>, @builtin(num_workgroups) ng: vec3<u32>) {
    ...
    if (!fluidParticleIsActive(i)) { return; }
    let pos = particles[i].position;
    let vi = clampCodecVelocity(particles[i].v, p);
    let speed = length(vi);
    ...
    for (var gz = 0; gz < 3; gz++) {
        ...
        let node = base + vec3<i32>(gx - 1, gy - 1, gz - 1);
        if (!inGrid(node, p)) { continue; }
        let m = decodeMass(cells[${cellIndex}].mass, p);
...
fn activeFlagBase(cap: u32) -> u32 { return 64u + 2u * activeStride(cap); }`
    ...
    let cap = arrayLength(&diffuse);
    let side = atomicLoad(&activeState[3]);
    let slot = linearDispatchIndex(gid, ng);
...
${slotLookup}
    ...
    for (var gz = 0; gz < 3; gz++) {
        ...
        if (!inGrid(node, p)) { continue; }
        let idx = ${cellIndex};
        vf += vec3<f32>(decodeMomentum(cells[idx].vx, p), decodeMomentum(cells[idx].vy, p), decodeMomentum(cells[idx].vz, p)) * weight;
        rho += decodeMass(cells[idx].mass, p) * weight;
    }}}
...
export interface MlsMpmOptions extends FluidSimBaseOptions {
    ...
    dx?: number;
    /** Per-particle world volume used for authored initial-state density and flow rates. */
    ...
    /** Called asynchronously with the latest completed active-page demand. */
     ...
     *  volume-sampled particle set (see fluid/sampling/volume). */
...
export function createMlsMpmSim(engine: EngineContext, options: MlsMpmOptions = {}): FluidSim {
    ...
    let stiffness = options.stiffness ?? 50;
    let viscosity = options.viscosity ?? 0.1;
    ...
    let restitution = Math.min(1, Math.max(0, options.restitution ?? 0.3));
    ...
    let restDensity = options.restDensity ?? Math.max(2, count / spawnVolCells);
    ...
    const workingParticleBuffer = device.createBuffer({
        label: "mpm-particles-working",
    ...
    const debugBuffer = device.createBuffer({ label: "mpm-debug", size: count * 4, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST | GPUBufferUsage.COPY_SRC });
    ...
    const flowState = createFluidFlowState(device, count, particleRadius, options.particleVolume);
    enableFluidActiveCountReadback(flowState);
    const workingLifecycleBuffer = device.createBuffer({
        label: "mpm-lifecycle-working",
        size: flowState.lifecycleBuffer.size,
        usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
    });
    const workingFlowCounterBuffer = device.createBuffer({
        label: "mpm-flow-counters-working",
        size: flowState.counterBuffer.size,
        usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
    });
    const workingFlowState = { ...flowState, lifecycleBuffer: workingLifecycleBuffer };
    const warmupState = createFluidWarmupState(workingFlowState);
    ...
    const cellCountBuffer = device.createBuffer({ label: "mpm-cell-count", size: numCells * 4, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST });
    ...
    const activeBlockIndirectBuffer = activeBlocks
        ...
        : null;
    const denseP2gIndirectBuffer = activeBlocks
        ? null
        : device.createBuffer({ label: "mpm-dense-p2g-indirect", size: 12, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.INDIRECT });
    ...
    const pageStatusStagingBuffers = pagedGrid
        ...
        : [];
    const accumulationStateBuffer = device.createBuffer({
        label: "mpm-accumulation-state",
        size: 16,
        usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST | GPUBufferUsage.COPY_SRC,
    });
    const accumulationStatusStagingBuffers = [0, 1].map((i) =>
        device.createBuffer({
            label: `mpm-accumulation-status-readback-${i}`,
            size: 8,
            usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
        })
    );
    ...
    function writeDynamicParams(subDt: number, frameDt: number): void {
        pf[16] = subDt;
        pf[MISC2_BASE_F32] = restitution;
        pf[MISC2_BASE_F32 + 1] = frameDt;
        const codec = createMlsFixedPointCodec({
            dx,
            subDt,
            restDensity,
            stiffness,
        ...
        pf[CODEC_BASE_F32] = codec.massScale;
        pf[CODEC_BASE_F32 + 1] = codec.inverseMassScale;
        pf[CODEC_BASE_F32 + 2] = codec.momentumScale;
        pf[CODEC_BASE_F32 + 3] = codec.inverseMomentumScale;
        pf[CODEC_LIMITS_BASE_F32] = codec.maxResolvedSpeed;
        pf[CODEC_LIMITS_BASE_F32 + 1] = codec.maxAffineFrobenius;
        pf[CODEC_LIMITS_BASE_F32 + 2] = codec.maxVelocityContribution;
        pf[CODEC_LIMITS_BASE_F32 + 3] = codec.densityRatioCap;
        pf[CODEC_EXTRA_BASE_F32] = codec.densityFloorRatio;
        pu[CODEC_COUNTS_BASE_F32] = codec.cellContributorLimit;
    }
    ...
    const finalizeCodecPipe = pipeline("mpm-finalize-codec", FINALIZE_CODEC_WGSL);
    ...
    const denseP2gFinalizePipe = activeBlocks ? null : pipeline("mpm-finalize-dense-p2g", DENSE_P2G_INDIRECT_WGSL);
    ...
    // update-grid + g2p pipelines are cached by WGSL source, including the null-scene
    // collision-free variants, so re-selecting equivalent specs does not recompile.
    const updatePipeCache = new Map<string, GPUComputePipeline>();
    function getUpdatePipe(scene: SceneSdfSpec | null): GPUComputePipeline {
    ...
    let updatePipe: GPUComputePipeline = getUpdatePipe(null);
    // Force-free: any external force runs as its own dedicated pass, so G2P is keyed
    // on the scene source alone and reuses the null-scene variant when collisions are off.
    const g2pPipeCache = new Map<string, GPUComputePipeline>();
    function getG2pPipe(scene: SceneSdfSpec | null): GPUComputePipeline {
    ...
    let g2pPipe: GPUComputePipeline = getG2pPipe(null);
    const commitFramePipe = pipeline("mpm-commit-frame", buildCommitFrameWgsl(pagedGrid));
    const copyPipe = pipeline("mpm-copy", buildCopyWgsl(pagedGrid));
    ...
    let accumulationOverflowed = false;
    let pageStatusGeneration = 0;
    let accumulationStatusGeneration = 0;
    let pendingAsyncError: Error | null = null;
    const timestepScheduler = createFluidTimestepScheduler();
    let disposed = false;
    function latchAsyncError(message: string, cause?: unknown): void {
        if (!pendingAsyncError) {
            pendingAsyncError = new Error(message, cause === undefined ? undefined : { cause });
        }
    }
    const pageStatusStates: Array<"idle" | "copied" | "mapping"> = pageStatusStagingBuffers.map(() => "idle");
    const pageStatusGenerations = pageStatusStagingBuffers.map(() => -1);
    const pageStatusFoamSides = pageStatusStagingBuffers.map(() => 0);
    const pageStatusSamples = pageStatusStagingBuffers.map(() => -1);
    let nextPageStatusSample = 0;
    let lastPublishedPageStatusSample = -1;
    function pollPagedGridStatus(): void {
        ...
        for (let i = 0; i < pageStatusStates.length; i++) {
            ...
            const generation = pageStatusGenerations[i]!;
            const foamSide = pageStatusFoamSides[i]!;
            const sample = pageStatusSamples[i]!;
            ...
            staging
                ...
                .then(() => {
                    ...
                    pageStatusGenerations[i] = -1;
                    pageStatusSamples[i] = -1;
                    if (!disposed && generation === pageStatusGeneration && sample > lastPublishedPageStatusSample) {
                        lastPublishedPageStatusSample = sample;
                        options.onPagedGridPages?.(requiredPages, pagedGridMaxPages);
                    }
                    if (!disposed && generation === pageStatusGeneration && overflow !== 0 && !pagedGridOverflowed) {
                        resetFluidTimestepScheduler(timestepScheduler);
                        publishFoamActiveSide(foamSide);
                ...
                .catch((error: unknown) => {
                    ...
                    if (!disposed && generation === pageStatusGeneration) {
                        resetFluidTimestepScheduler(timestepScheduler);
                        publishFoamActiveSide(foamSide);
                        latchAsyncError("[MLS-MPM] Failed to read paged-grid overflow status.", error);
                    }
                });
        }
    }
    const accumulationStatusStates: Array<"idle" | "copied" | "mapping"> = accumulationStatusStagingBuffers.map(() => "idle");
    const accumulationStatusGenerations = accumulationStatusStagingBuffers.map(() => -1);
    const accumulationStatusFoamSides = accumulationStatusStagingBuffers.map(() => 0);
    function pollAccumulationStatus(): void {
        for (let i = 0; i < accumulationStatusStates.length; i++) {
            if (accumulationStatusStates[i] !== "copied") {
                continue;
            }
            accumulationStatusStates[i] = "mapping";
            const generation = accumulationStatusGenerations[i]!;
            const foamSide = accumulationStatusFoamSides[i]!;
            const staging = accumulationStatusStagingBuffers[i]!;
            staging
                .mapAsync(GPUMapMode.READ)
                .then(() => {
                    const status = new Uint32Array(staging.getMappedRange());
                    const overflow = status[0]!;
                    const observed = status[1]!;
                    staging.unmap();
                    accumulationStatusStates[i] = "idle";
                    accumulationStatusGenerations[i] = -1;
                    if (!disposed && generation === accumulationStatusGeneration && overflow !== 0 && !accumulationOverflowed) {
                        resetFluidTimestepScheduler(timestepScheduler);
                        publishFoamActiveSide(foamSide);
                        accumulationOverflowed = true;
                        latchAsyncError(
                            `[MLS-MPM] Fixed-point accumulation stopped: no overflow-safe 32-bit codec exists for an observed P2G base-cell population of ${observed} under the current timestep and material bounds.`
                        );
                    }
                })
                .catch((error: unknown) => {
                    accumulationStatusStates[i] = "idle";
                    accumulationStatusGenerations[i] = -1;
                    if (!disposed && generation === accumulationStatusGeneration) {
                        resetFluidTimestepScheduler(timestepScheduler);
                        publishFoamActiveSide(foamSide);
                        latchAsyncError("[MLS-MPM] Failed to read fixed-point accumulation status.", error);
                    }
                });
        }
    }
    const flowEntries: GPUBindGroupEntry[] = [
        { binding: 0, resource: { buffer: workingParticleBuffer } },
        { binding: 1, resource: { buffer: flowState.uniformBuffer } },
        { binding: 2, resource: { buffer: workingFlowCounterBuffer } },
        { binding: 3, resource: { buffer: workingLifecycleBuffer } },
    ];
    ...
    histogramEntries.push(
        { binding: fusedBlockDiscovery ? 5 : 3, resource: { buffer: workingLifecycleBuffer } },
        { binding: fusedBlockDiscovery ? 6 : 4, resource: { buffer: accumulationStateBuffer } },
        { binding: fusedBlockDiscovery ? 7 : 5, resource: { buffer: cellCountBuffer } }
    );
    const histogramBG = device.createBindGroup({
        layout: histogramPipe.getBindGroupLayout(0),
        entries: histogramEntries,
    });
    const finalizeCodecBG = device.createBindGroup({
        layout: finalizeCodecPipe.getBindGroupLayout(0),
        entries: [
            { binding: 0, resource: { buffer: paramsBuffer } },
            { binding: 1, resource: { buffer: accumulationStateBuffer } },
        ],
    });
    ...
    const scatterBG = device.createBindGroup({
        ...
        entries: [
            ...
            { binding: 5, resource: { buffer: workingLifecycleBuffer } },
        ],
    });
    ...
    const finalizeActiveBlocksBG = activeBlocks
        ? device.createBindGroup({
              ...
              entries: [
                  ...
                  { binding: 2, resource: { buffer: accumulationStateBuffer } },
              ],
          })
        : null;
    ...
    const finalizeNodeBlocksBG = activeBlocks
        ...
        : null;
    const finalizeDenseP2gBG = activeBlocks
        ? null
        : device.createBindGroup({
              layout: denseP2gFinalizePipe!.getBindGroupLayout(0),
              entries: [
                  { binding: 0, resource: { buffer: denseP2gIndirectBuffer! } },
                  { binding: 1, resource: { buffer: accumulationStateBuffer } },
                  { binding: 2, resource: { buffer: paramsBuffer } },
              ],
          });
    const assignGridPagesBG = pagedGrid
        ...
        : null;
    function buildUpdateBG(pipe: GPUComputePipeline, scene: SceneSdfSpec | null): GPUBindGroup {
        ...
        if (scene && scene.gridConfine !== false) {
        ...
        entries.push({ binding: 8, resource: { buffer: accumulationStateBuffer } });
        return device.createBindGroup({ layout: pipe.getBindGroupLayout(0), entries });
    }
    let updateBG: GPUBindGroup = buildUpdateBG(updatePipe, null);
    function buildG2pBG(pipe: GPUComputePipeline, scene: SceneSdfSpec | null): GPUBindGroup {
        const entries: GPUBindGroupEntry[] = [
            { binding: 0, resource: { buffer: workingParticleBuffer } },
            { binding: 1, resource: { buffer: cellBuffer } },
            { binding: 2, resource: { buffer: paramsBuffer } },
        ...
        entries.push({ binding: pagedGrid ? 7 : scene?.sdfGrid ? 5 : 4, resource: { buffer: workingLifecycleBuffer } });
        entries.push({ binding: 8, resource: { buffer: accumulationStateBuffer } });
        return device.createBindGroup({ layout: pipe.getBindGroupLayout(0), entries });
    }
    let g2pBG: GPUBindGroup = buildG2pBG(g2pPipe, null);
    ...
    const copyBG = device.createBindGroup({
        ...
        entries: [
            ...
            { binding: 5, resource: { buffer: workingLifecycleBuffer } },
            { binding: 6, resource: { buffer: accumulationStateBuffer } },
            ...(pagedGrid ? [{ binding: 7, resource: { buffer: activeBlockCountBuffer! } }] : []),
        ],
    });
    const commitFrameBG = device.createBindGroup({
        layout: commitFramePipe.getBindGroupLayout(0),
        entries: [
            { binding: 0, resource: { buffer: workingParticleBuffer } },
            { binding: 1, resource: { buffer: particleBuffer } },
            { binding: 2, resource: { buffer: workingLifecycleBuffer } },
            { binding: 3, resource: { buffer: flowState.lifecycleBuffer } },
            { binding: 4, resource: { buffer: accumulationStateBuffer } },
            ...(pagedGrid ? [{ binding: 5, resource: { buffer: activeBlockCountBuffer! } }] : []),
        ],
    });
    ...
    function publishFoamActiveSide(side: number): void {
        foamActiveSide = side;
        if (!foamActiveParticles || !diffuseBuffer || !foamActiveStateBuffer || !foamDrawIndirectBuffer) {
            return;
        }
        diffusePool = {
    ...
    function buildFoamBindGroups(): void {
        foamEmitBG = device.createBindGroup({
            ...
            entries: [
                ...
                ...(pagedGrid
                    ...
                    : []),
                { binding: pagedGrid ? 8 : 6, resource: { buffer: workingLifecycleBuffer } },
                { binding: 9, resource: { buffer: accumulationStateBuffer } },
            ],
        });
        ...
        updateEntries.push({ binding: 8, resource: { buffer: accumulationStateBuffer } });
        ...
        foamActivePrepareBG = foamActiveParticles
            ? device.createBindGroup({
                  ...
                  entries: [
                      ...
                      { binding: 2, resource: { buffer: accumulationStateBuffer } },
                      ...(pagedGrid ? [{ binding: 3, resource: { buffer: activeBlockCountBuffer! } }] : []),
                  ],
              })
            : null;
        foamActiveFinishBG = foamActiveParticles
            ? device.createBindGroup({
                  ...
                  entries: [
                      ...
                      { binding: 3, resource: { buffer: accumulationStateBuffer } },
                      ...(pagedGrid ? [{ binding: 4, resource: { buffer: activeBlockCountBuffer! } }] : []),
                  ],
              })
            : null;
    }
    ...
    function ensureFoam(cfg: FoamConfig): void {
        if (!foamDenseEmitPipe) {
            const nextFoamDenseEmitPipe = pipeline("mpm-foam-emit", buildFoamEmitWgsl(false, pagedGrid));
            const nextFoamDenseUpdatePipe = pipeline("mpm-foam-update", buildFoamUpdateWgsl(false, pagedGrid));
            const [nextFoamParamsBuffer, nextDiffuseHeadBuffer] = createGpuBuffersAtomically(device, [
                { label: "mpm-foam-params", size: FOAM_BYTES, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST },
                { label: "mpm-foam-head", size: 16, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST },
            ]);
            foamDenseEmitPipe = nextFoamDenseEmitPipe;
            foamDenseUpdatePipe = nextFoamDenseUpdatePipe;
        ...
        if (nextActiveParticles && !foamActiveUpdatePipe) {
            const nextFoamActiveEmitPipe = pipeline("mpm-foam-emit-active", buildFoamEmitWgsl(true, pagedGrid));
            const nextFoamActiveUpdatePipe = pipeline("mpm-foam-update-active", buildFoamUpdateWgsl(true, pagedGrid));
            const nextFoamActivePreparePipe = pipeline("mpm-foam-active-prepare", buildFoamActivePrepareWgsl(pagedGrid));
            const nextFoamActiveFinishPipe = pipeline("mpm-foam-active-finish", buildFoamActiveFinishWgsl(pagedGrid));
            foamActiveEmitPipe = nextFoamActiveEmitPipe;
            foamActiveUpdatePipe = nextFoamActiveUpdatePipe;
            foamActivePreparePipe = nextFoamActivePreparePipe;
            foamActiveFinishPipe = nextFoamActiveFinishPipe;
        }
        ...
        const diffuseIndex = resizePool ? descriptors.push({ label: "mpm-foam-pool", size: cap * 32, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST }) - 1 : -1;
        const activeIndex = rebuildActiveResources
            ? descriptors.push(
                  { label: "mpm-foam-active-state", size: foamActiveStateBytes(cap), usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST },
                  { label: "mpm-foam-active-dispatch", size: 12, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.INDIRECT },
                  { label: "mpm-foam-draw-indirect", size: 16, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.INDIRECT | GPUBufferUsage.COPY_DST }
              ) - 3
            : -1;
    ...
    return {
        ...
        get gpuBytes(): number {
            ...
            let b =
                ...
                workingParticleBuffer.size +
                workingLifecycleBuffer.size +
                workingFlowCounterBuffer.size;
            b += accumulationStateBuffer.size;
            for (const buffer of accumulationStatusStagingBuffers) {
                b += buffer.size;
            }
            // Block counting-sort buffers (always allocated).
            b += cellCountBuffer.size + blockCountBuffer.size + blockStartBuffer.size + blockCursorBuffer.size + partialSumsBuffer.size + sortedIdxBuffer.size;
            if (denseP2gIndirectBuffer) {
                b += denseP2gIndirectBuffer.size;
            }
        ...
        step(encoder: GPUCommandEncoder, dt: number): void {
            ...
            pollPagedGridStatus();
            pollAccumulationStatus();
            pollFluidActiveCount(flowState);
            if (pagedGridOverflowed || accumulationOverflowed) {
                return;
            }
            if (!Number.isFinite(dt) || dt <= 0) {
                return;
            }
            const pageStagingIndex = pagedGrid ? pageStatusStates.indexOf("idle") : -1;
            const accumulationStagingIndex = accumulationStatusStates.indexOf("idle");
            if ((pagedGrid && pageStagingIndex === -1) || accumulationStagingIndex === -1) {
                deferFluidTimestep(timestepScheduler, dt);
                return;
            }
            const frameFoamActiveSide = foamActiveSide;
            ...
            writeDynamicParams(schedule.substepDeltaSeconds, frameDt);
            encoder.copyBufferToBuffer(particleBuffer, 0, workingParticleBuffer, 0, particleBuffer.size);
            encoder.copyBufferToBuffer(flowState.lifecycleBuffer, 0, workingLifecycleBuffer, 0, flowState.lifecycleBuffer.size);
            if (liveCount < initialTargetCount) {
                const previous = liveCount;
                liveCount = Math.min(initialTargetCount, liveCount + warmupStep);
                encodeFluidWarmup(workingFlowState, warmupState, encoder, previous, liveCount);
            }
            const flowFrame = prepareFluidFlowFrame(flowState, frameDt);
            device.queue.writeBuffer(workingFlowCounterBuffer, 0, flowState.counterData);
            pu[COUNTS_OFFSET_F32 + 2] = count;
            device.queue.writeBuffer(paramsBuffer, 0, paramsData);
            encoder.clearBuffer(accumulationStateBuffer);
            ...
            for (let s = 0; s < stepCount; s++) {
                ...
                encoder.clearBuffer(cellCountBuffer);
                encoder.clearBuffer(blockCursorBuffer);
                encoder.clearBuffer(accumulationStateBuffer, 8, 4);
                if (activeBlocks) {
                ...
                dispatch(encoder, "mpm-finalize-codec", finalizeCodecPipe, finalizeCodecBG, 1);
                if (activeBlocks) {
                    if (!fusedBlockDiscovery) {
                    ...
                    dispatch(encoder, "mpm-finalize-dense-p2g", denseP2gFinalizePipe!, finalizeDenseP2gBG!, 1);
                }
                ...
                if (activeBlocks) {
                    ...
                    dispatchIndirect(encoder, "mpm-p2g-mass", p2gMassTiledPipe, p2gMassTiledBG, denseP2gIndirectBuffer!);
                    dispatchIndirect(encoder, "mpm-p2g-vel", p2gVelTiledPipe, p2gVelTiledBG, denseP2gIndirectBuffer!);
                }
                if (activeBlocks) {
            ...
            dispatch(encoder, "mpm-commit-frame", commitFramePipe, commitFrameBG, particleGroups);
            ...
            if (foamEnabled && foamUpdateBG) {
                ...
                if (foamActiveParticles) {
                    ...
                    publishFoamActiveSide(1 - foamActiveSide);
                } else {
            ...
            if (pagedGrid) {
                encoder.copyBufferToBuffer(activeBlockCountBuffer!, 4, pageStatusStagingBuffers[pageStagingIndex]!, 0, 8);
                pageStatusGenerations[pageStagingIndex] = pageStatusGeneration;
                pageStatusFoamSides[pageStagingIndex] = frameFoamActiveSide;
                pageStatusSamples[pageStagingIndex] = nextPageStatusSample++;
                pageStatusStates[pageStagingIndex] = "copied";
            }
            encoder.copyBufferToBuffer(accumulationStateBuffer, 0, accumulationStatusStagingBuffers[accumulationStagingIndex]!, 0, 8);
            accumulationStatusGenerations[accumulationStagingIndex] = accumulationStatusGeneration;
            accumulationStatusFoamSides[accumulationStagingIndex] = frameFoamActiveSide;
            accumulationStatusStates[accumulationStagingIndex] = "copied";
            encoder.popDebugGroup();
        },
        ...
        reset(): void {
            ...
            accumulationOverflowed = false;
            accumulationStatusGeneration++;
            resetFluidTimestepScheduler(timestepScheduler);
            for (let i = 0; i < accumulationStatusStates.length; i++) {
                if (accumulationStatusStates[i] === "copied") {
                    accumulationStatusStates[i] = "idle";
                    accumulationStatusGenerations[i] = -1;
                    accumulationStatusFoamSides[i] = 0;
                }
            }
            pendingAsyncError = null;
            const accumulationReset = device.createCommandEncoder({ label: "mpm-accumulation-reset" });
            accumulationReset.clearBuffer(accumulationStateBuffer);
            device.queue.submit([accumulationReset.finish()]);
            if (pagedGrid) {
                pagedGridOverflowed = false;
                pageStatusGeneration++;
                lastPublishedPageStatusSample = -1;
                for (let i = 0; i < pageStatusStates.length; i++) {
                    if (pageStatusStates[i] === "copied") {
                        pageStatusStates[i] = "idle";
                        pageStatusGenerations[i] = -1;
                        pageStatusFoamSides[i] = 0;
                        pageStatusSamples[i] = -1;
                    }
                }
        ...
        dispose(): void {
            ...
            workingLifecycleBuffer.destroy();
            workingFlowCounterBuffer.destroy();
            ...
            denseP2gIndirectBuffer?.destroy();
            ...
            accumulationStateBuffer.destroy();
            for (const buffer of accumulationStatusStagingBuffers) {
                buffer.destroy();
            }
