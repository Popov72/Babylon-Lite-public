...
import { buildLinearDispatchIndexWgsl } from "./wgsl-shared.js";
...
struct Params {
    ...
    misc: vec4<f32>,      // frameDt, restitution, represented particle volume, _
};

fn enc(x: f32) -> i32 { return i32(clamp(x, -MAX_ENCODE, MAX_ENCODE) * FIXED_POINT); }
fn dec(x: i32) -> f32 { return f32(x) * FIXED_POINT_INV; }
fn reflectMotion(v: vec3<f32>, n: vec3<f32>, e: f32) -> vec3<f32> {
    let vn = dot(v, n);
    if (vn <= 0.0) { return v; }
    return v - (1.0 + e) * vn * n;
}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>, @builtin(num_workgroups) ng: vec3<u32>) {
    ...
    for (var gz = 0; gz < 3; gz++) {
        ...
        if (part.material < 0.5) {
            atomicAdd(&volumes[idx], enc(weight * p.misc.z));
        }
    }}}
}`;
...
function buildIntegrateWgsl(scene: SceneSdfSpec | null): string {
    ...
    if (sd < 0.0) {
        ...
        np = np - sd * n;
        nextDisp = reflectMotion(nextDisp, -n, p.misc.y);
    }`
        : "";
    return /* wgsl */ `
${SVD3_WGSL}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>, @builtin(num_workgroups) ng: vec3<u32>) {
    ...
    var nextDisp = part.displacement;
    if (part.material < 0.5) {
...
${sceneResolve}
    let lo = p.boundsMin.xyz + vec3<f32>(1.0e-4);
    let hi = p.boundsMax.xyz - vec3<f32>(1.0e-4);
    if (np.x < lo.x) {
        np.x = lo.x;
        nextDisp = reflectMotion(nextDisp, vec3<f32>(-1.0, 0.0, 0.0), p.misc.y);
    } else if (np.x > hi.x) {
        np.x = hi.x;
        nextDisp = reflectMotion(nextDisp, vec3<f32>(1.0, 0.0, 0.0), p.misc.y);
    }
    if (np.y < p.dim.w + 1.0e-4) {
        np.y = p.dim.w + 1.0e-4;
        nextDisp = reflectMotion(nextDisp, vec3<f32>(0.0, -1.0, 0.0), p.misc.y);
    } else if (np.y > hi.y) {
        np.y = hi.y;
        nextDisp = reflectMotion(nextDisp, vec3<f32>(0.0, 1.0, 0.0), p.misc.y);
    }
    if (np.z < lo.z) {
        np.z = lo.z;
        nextDisp = reflectMotion(nextDisp, vec3<f32>(0.0, 0.0, -1.0), p.misc.y);
    } else if (np.z > hi.z) {
        np.z = hi.z;
        nextDisp = reflectMotion(nextDisp, vec3<f32>(0.0, 0.0, 1.0), p.misc.y);
    }
    nextDisp = clampLen(nextDisp, p.origin.w * 0.9) * DISPLACEMENT_DAMPING;
...
export interface PbMpmOptions extends FluidSimBaseOptions {
    ...
    dx?: number;
    /** Nominal per-particle world volume used for authored initial-state density and flow rates. */
    ...
    material?: number;
    /** Collision restitution for scene and domain collisions. Default 0. */
    restitution?: number;
...
export function createPbMpmSim(engine: EngineContext, options: PbMpmOptions = {}): FluidSim {
    ...
    const nominalParticleVolume = Math.max(options.particleVolume ?? (particleRadius * 2) ** 3, 1e-12);
    ...
    let restitution = Math.min(1, Math.max(0, options.restitution ?? 0));
    ...
    const debugBuffer = device.createBuffer({ label: "pbmpm-debug", size: count * 4, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST | GPUBufferUsage.COPY_SRC });
    ...
    const flowState = createFluidFlowState(device, count, particleRadius, nominalParticleVolume);
    ...
    pf[13] = boundsMax[1];
    pf[14] = boundsMax[2];
    pf[MISC_OFFSET_F32] = 1 / 60;
    pf[MISC_OFFSET_F32 + 1] = restitution;
    pf[MISC_OFFSET_F32 + 2] = nominalParticleVolume;
    ...
    function seed(): void {
        ...
        const flowParticles =
            initialPositions || !flowSeedsInitialParticles ? null : createFluidInitialParticles(count, flowState.config, nominalParticleVolume, { min: boundsMin, max: boundsMax });
    ...
    const gridUpdatePipeCache = new Map<string, GPUComputePipeline>();
    function getGridUpdatePipe(scene: SceneSdfSpec | null): GPUComputePipeline {
        const src = buildGridUpdateWgsl(scene);
        let pipe = gridUpdatePipeCache.get(src);
        if (!pipe) {
            pipe = pipeline("pbmpm-grid-update", src);
            gridUpdatePipeCache.set(src, pipe);
        }
        return pipe;
    }
    const integratePipeCache = new Map<string, GPUComputePipeline>();
    function getIntegratePipe(scene: SceneSdfSpec | null): GPUComputePipeline {
        const src = buildIntegrateWgsl(scene);
        let pipe = integratePipeCache.get(src);
        if (!pipe) {
            pipe = pipeline("pbmpm-integrate", src);
            integratePipeCache.set(src, pipe);
        }
        return pipe;
    }
    let gridUpdatePipe = getGridUpdatePipe(null);
    let integratePipe = getIntegratePipe(null);
    ...
    function ensureFoam(cfg: FoamConfig): void {
        if (!foamDenseEmitPipe) {
            const nextFoamDenseEmitPipe = pipeline("pbmpm-foam-emit", buildFoamEmitWgsl(false));
            const nextFoamDenseUpdatePipe = pipeline("pbmpm-foam-update", buildFoamUpdateWgsl(false));
            const [nextFoamParamsBuffer, nextDiffuseHeadBuffer] = createGpuBuffersAtomically(device, [
                { label: "pbmpm-foam-params", size: FOAM_BYTES, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST },
                { label: "pbmpm-foam-head", size: 16, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST },
            ]);
        ...
        if (nextActiveParticles && !foamActiveUpdatePipe) {
            const nextFoamActiveEmitPipe = pipeline("pbmpm-foam-emit-active", buildFoamEmitWgsl(true));
            const nextFoamActiveUpdatePipe = pipeline("pbmpm-foam-update-active", buildFoamUpdateWgsl(true));
            const nextFoamActivePreparePipe = pipeline("pbmpm-foam-active-prepare", FOAM_ACTIVE_PREPARE_WGSL);
            const nextFoamActiveFinishPipe = pipeline("pbmpm-foam-active-finish", FOAM_ACTIVE_FINISH_WGSL);
        ...
        const diffuseIndex = resizePool ? descriptors.push({ label: "pbmpm-foam-pool", size: cap * 32, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST }) - 1 : -1;
        const activeIndex = rebuildActiveResources
            ? descriptors.push(
                  { label: "pbmpm-foam-active-state", size: foamActiveStateBytes(cap), usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST },
                  { label: "pbmpm-foam-active-dispatch", size: 12, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.INDIRECT },
                  { label: "pbmpm-foam-draw-indirect", size: 16, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.INDIRECT | GPUBufferUsage.COPY_DST }
              ) - 3
            : -1;
    ...
    return {
        ...
        setSceneSdf(spec: SceneSdfSpec | null): void {
            gridUpdatePipe = getGridUpdatePipe(spec);
            integratePipe = getIntegratePipe(spec);
