...
export function createFloatingBodySystem(device: GPUDevice, opts: FloatingBodySystemOptions): FloatingBodySystem {
    ...
    const setBodyPose = (
        ...
        const ql = Math.sqrt(quaternion[0] * quaternion[0] + quaternion[1] * quaternion[1] + quaternion[2] * quaternion[2] + quaternion[3] * quaternion[3]);
        ...
        for (let i = 0; i < bodies.length; i++) {
            ...
            if (enabled) {
                ...
                if (!b.externalPose) {
                    ...
                    const nl = Math.sqrt(nqx * nqx + nqy * nqy + nqz * nqz + nqw * nqw) || 1;
                    nqx /= nl;
                    nqy /= nl;
                    nqz /= nl;
                    nqw /= nl;
