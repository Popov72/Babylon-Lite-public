...
/** Opt-in GPU timing hook. The concrete implementation lives in
...
export interface FluidSim {
    ...
    /** f32-per-particle speed (Phase 3 debug). Read by the renderer and copied by
     * aggregate collection streams, so backend buffers require COPY_SRC usage. */
    ...
    /** Bounded catch-up state for delayed or blocked frame deltas. */
    readonly timestepDiagnostics?: FluidTimestepDiagnostics;
    ...
    /** Materialize all currently enabled optional resources before a facade candidate is exposed. */
    prepare?(): void;
...
/**
 * Allocates a related buffer set as one failure-atomic operation. Callers assign
 * the returned buffers only after this function succeeds, so current live
 * resources remain usable if a later allocation in the set fails.
 */
export function createGpuBuffersAtomically(device: GPUDevice, descriptors: readonly GPUBufferDescriptor[]): GPUBuffer[] {
    const allocated: GPUBuffer[] = [];
    try {
        for (const descriptor of descriptors) {
            allocated.push(device.createBuffer(descriptor));
        }
        return allocated;
    } catch (error) {
...
export interface DiffuseCountTracker {
    ...
    restore(counts: DiffuseParticleCounts | undefined): void;
...
export function createDiffuseCountTracker(device: GPUDevice, label: string): DiffuseCountTracker {
    const trackerBuffers = createGpuBuffersAtomically(device, [
        {
            label: `${label}-counts`,
            size: 16,
            usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC | GPUBufferUsage.COPY_DST,
        },
        ...[0, 1].map((index): GPUBufferDescriptor => ({
            label: `${label}-counts-readback-${index}`,
            size: 16,
            usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ,
        })),
    ]);
    const counterBuffer = trackerBuffers[0]!;
    const readbacks = trackerBuffers.slice(1);
    let densePipeline: GPUComputePipeline;
    let activePipeline: GPUComputePipeline;
    try {
        densePipeline = device.createComputePipeline({
            label: `${label}-count-dense`,
            layout: "auto",
            compute: { module: device.createShaderModule({ code: DIFFUSE_COUNT_DENSE_WGSL }), entryPoint: "main" },
        });
        activePipeline = device.createComputePipeline({
            label: `${label}-count-active`,
            layout: "auto",
            compute: { module: device.createShaderModule({ code: DIFFUSE_COUNT_ACTIVE_WGSL }), entryPoint: "main" },
        });
    } catch (error) {
        for (const buffer of trackerBuffers) {
            buffer.destroy();
        }
        throw error;
    }
    ...
    return {
        ...
        configure(diffuse: GPUBuffer, nextCapacity: number, activeState?: GPUBuffer): void {
            ...
            const nextDenseBindGroup = device.createBindGroup({
            ...
            const nextActiveBindGroup = activeState
            ...
            denseBindGroup = nextDenseBindGroup;
            activeBindGroup = nextActiveBindGroup;
            latest = undefined;
            frame = 0;
        },
        encode(encoder: GPUCommandEncoder, denseGroups: number, activeDispatch?: GPUBuffer): void {
        ...
        restore(counts: DiffuseParticleCounts | undefined): void {
            poll();
            latest = counts ? { ...counts, capacity } : undefined;
        },
...
export interface FluidSink {
    ...
    transform: FluidTransform;
    shape: FluidShape;
    /** Simulation-time seconds before capture starts. Defaults to 0. */
    delayBeforeStart?: number;
...
function sampleLocalShape(shape: FluidShape, sampling: "volume" | "surface"): FluidVec3 {
    switch (shape.type) {
        ...
        case "cone": {
            ...
            const lateral = Math.PI * (r0 + r1) * Math.sqrt(delta * delta + height * height);
...
function localShapeNormal(shape: FluidShape, point: FluidVec3): FluidVec3 {
    switch (shape.type) {
        ...
        case "cylinder": {
            ...
            const radial = Math.sqrt(point[0] * point[0] + point[2] * point[2]);
        ...
        case "cone": {
            ...
            const lateralDistance = Math.abs(radius - radial) / Math.sqrt(1 + slope * slope);
...
function normalisedRotation(rotation: FluidTransform["rotation"]): FluidTransform["rotation"] {
    const length = Math.sqrt(rotation[0] * rotation[0] + rotation[1] * rotation[1] + rotation[2] * rotation[2] + rotation[3] * rotation[3]);
    return length > 1e-8 ? [rotation[0] / length, rotation[1] / length, rotation[2] / length, rotation[3] / length] : [0, 0, 0, 1];
}

function rotateVectorWithNormalisedRotation(rotation: FluidTransform["rotation"], value: FluidVec3): FluidVec3 {
...
function rotateVector(rotation: FluidTransform["rotation"], value: FluidVec3): FluidVec3 {
    return rotateVectorWithNormalisedRotation(normalisedRotation(rotation), value);
}

function transformFluidPointWithNormalisedRotation(transform: FluidTransform, rotation: FluidTransform["rotation"], local: FluidVec3): FluidVec3 {
    const scaled: FluidVec3 = [local[0] * transform.scale[0], local[1] * transform.scale[1], local[2] * transform.scale[2]];
    const rotated = rotateVectorWithNormalisedRotation(rotation, scaled);
...
function transformFluidPoint(transform: FluidTransform, local: FluidVec3): FluidVec3 {
    return transformFluidPointWithNormalisedRotation(transform, normalisedRotation(transform.rotation), local);
}
...
function fluidEmitterLaunchAtLocal(emitter: FluidEmitter, local: FluidVec3): { position: FluidVec3; velocity: FluidVec3 } {
    ...
    const spread = Math.sqrt(base[0] * base[0] + base[1] * base[1] + base[2] * base[2]) * emitter.spread;
    return {
...
function localShapeBounds(shape: FluidShape): { min: FluidVec3; max: FluidVec3 } {
    switch (shape.type) {
        ...
        case "capsule": {
            const radius = absFinite(shape.radius);
            const halfHeight = Math.max(radius, absFinite(shape.height) * 0.5);
            return { min: [-radius, -halfHeight, -radius], max: [radius, halfHeight, radius] };
        }
        case "polygonPrism": {
            const halfThickness = absFinite(shape.thickness) * 0.5;
...
function createFluidInitialLattice(emitter: FluidEmitter, count: number, worldVolume: number, worldBounds?: FluidInitialBounds): FluidVec3[] {
    ...
    const rotation = normalisedRotation(emitter.transform.rotation);
            ...
            for (let z = 0; z < dimensions[2]; z++) {
                for (let x = 0; x < dimensions[0]; x++) {
                    ...
                    if (localShapeContains(emitter.shape, point, polygon)) {
                        unclippedCount++;
                        if (!worldBounds || fluidPointInsideBounds(transformFluidPointWithNormalisedRotation(emitter.transform, rotation, point), worldBounds)) {
...
export function countFluidInitialParticles(
    ...
    for (let index = 0; index < allocations.length; index++) {
        ...
        const accepted =
            emitter.sampling === "volume"
                ? Math.min(allocation, countFluidInitialLattice(emitter, Math.max(allocation, densityAllocations[index]!), volumes[index]!, bounds))
                : allocation;
...
export function createFluidInitialParticles(
    ...
    for (let allocationIndex = 0; allocationIndex < allocations.length; allocationIndex++) {
        ...
        const localPoints =
            emitter.sampling === "volume"
                ? createFluidInitialLattice(emitter, Math.max(allocation, densityAllocations[allocationIndex]!), volumes[allocationIndex]!, bounds)
                : undefined;
        const launchCount = localPoints ? Math.min(allocation, localPoints.length) : allocation;
...
export interface FluidFlowCarry {
    readonly id: string;
    readonly value: number;
}

export function captureFluidFlowCarries(ids: readonly string[], carries: Float64Array): FluidFlowCarry[] {
    return ids.map((id, index) => ({ id, value: carries[index] ?? 0 }));
}

export function restoreFluidFlowCarries(ids: readonly string[], carries: Float64Array, saved: readonly FluidFlowCarry[]): void {
    const byId = new Map(saved.map(({ id, value }) => [id, value]));
    carries.fill(0);
    for (let index = 0; index < ids.length && index < carries.length; index++) {
        carries[index] = byId.get(ids[index]!) ?? 0;
    }
}
...
export function setFluidFlowConfig(state: FluidFlowState, config: FluidFlowConfig | null): void {
    ...
    for (const sink of config?.sinks ?? []) {
        ...
        if (sink.delayBeforeStart !== undefined && (!Number.isFinite(sink.delayBeforeStart) || sink.delayBeforeStart < 0)) {
            throw new RangeError(`Fluid sink "${sink.id}" delayBeforeStart must be a finite non-negative number.`);
        }
    }
    ...
    const previousEmitterCarries = captureFluidFlowCarries(state.emitterIds, state.emitterCarries);
    const previousSinkCarries = captureFluidFlowCarries(state.sinkIds, state.sinkCarries);
    state.config = config;
    ...
    for (let i = 0; i < sinks.length; i++) {
        ...
        const active = sink.enabled && sinkPacked && (operation === FLOW_SINK_OPERATION_DELETE || targetMask !== 0);
        state.u32.set([active ? 1 : 0, targetMask >>> 0, sink.volumeRate === undefined ? UNLIMITED_FLOW_BUDGET : 0, i], offset + 24);
        ...
        state.sinkActive.push(active);
        state.sinkDelays.push(sink.delayBeforeStart ?? 0);
        state.active ||= active;
    }
    restoreFluidFlowCarries(state.emitterIds, state.emitterCarries, previousEmitterCarries);
    restoreFluidFlowCarries(state.sinkIds, state.sinkCarries, previousSinkCarries);
...
export function prepareFluidFlowFrame(state: FluidFlowState, dt: number, velocityScale = 1): FluidFlowFrame {
    ...
    for (let i = 0; i < state.sinkRates.length; i++) {
        const sinkOffset = FLOW_SINK_BASE + i * FLOW_ENTITY_FLOATS;
        const budgetOffset = sinkOffset + 26;
        if (!state.sinkActive[i]) {
            state.u32[sinkOffset + 24] = 0;
            state.u32[budgetOffset] = 0;
            state.f32[sinkOffset + 31] = 0;
            continue;
        }
        const delay = state.sinkDelays[i] ?? 0;
        const activeDt = Math.max(0, frameEnd - Math.max(frameStart, delay));
        const started = delay <= frameStart || activeDt > 0;
        state.u32[sinkOffset + 24] = started ? 1 : 0;
        state.f32[sinkOffset + 31] = activeDt;
        if (!started) {
            state.u32[budgetOffset] = 0;
            continue;
        }
        ...
        const budget = fluidVolumeBudget(rate, activeDt, state.particleVolume, state.sinkCarries[i]!);
    ...
    return { flowActive: sinkStarted || emitActive, deleteActive: sinkStarted, emitActive, emitCount, emitUnlimited };
}
...
/** Shared authoritative analytical-shape tests. Requires global `flow`. */
export const FLUID_FLOW_SHAPE_WGSL = /* wgsl */ `
...
fn fluidShapeLocalBounds(s: FluidShapeData) -> mat2x3<f32> {
    let kind=u32(s.scaleKind.w);
    if(kind==0u){let h=0.5*abs(s.params0.xyz);return mat2x3<f32>(-h,h);}
    if(kind==1u){let r=abs(s.params0.x);return mat2x3<f32>(vec3<f32>(-r),vec3<f32>(r));}
    if(kind==2u){let r=abs(s.params0.x);let h=0.5*abs(s.params0.y);return mat2x3<f32>(vec3<f32>(-r,-h,-r),vec3<f32>(r,h,r));}
    if(kind==3u){
        let r=max(abs(s.params0.x),abs(s.params0.y));let h=0.5*abs(s.params0.z);
        return mat2x3<f32>(vec3<f32>(-r,-h,-r),vec3<f32>(r,h,r));
    }
    if(kind==4u){
        let r=abs(s.params0.x);let h=max(r,0.5*abs(s.params0.y));
        return mat2x3<f32>(vec3<f32>(-r,-h,-r),vec3<f32>(r,h,r));
    }
    let h=0.5*abs(s.params0.x);
    if(s.polygon.w==0u){return mat2x3<f32>(vec3<f32>(0.0,-h,0.0),vec3<f32>(0.0,h,0.0));}
    var lo=vec2<f32>(1e20);var hi=vec2<f32>(-1e20);
    for(var i=0u;i<s.polygon.w;i=i+1u){let p=flow.points[s.polygon.z+i].xy;lo=min(lo,p);hi=max(hi,p);}
    return mat2x3<f32>(vec3<f32>(lo.x,-h,lo.y),vec3<f32>(hi.x,h,hi.y));
}
fn fluidShapeWorldBounds(s: FluidShapeData) -> mat2x3<f32> {
    let bounds=fluidShapeLocalBounds(s);var lo=vec3<f32>(1e20);var hi=vec3<f32>(-1e20);
    for(var corner=0u;corner<8u;corner=corner+1u){
        ...
        let world=fluidToWorld(s,local);lo=min(lo,world);hi=max(hi,world);
    }
    return mat2x3<f32>(lo,hi);
}
fn fluidCross2(a:vec2<f32>,b:vec2<f32>)->f32{return a.x*b.y-a.y*b.x;}
fn fluidOnSegment(a:vec2<f32>,b:vec2<f32>,p:vec2<f32>)->bool{
    return abs(fluidCross2(b-a,p-a))<=1e-6&&all(p>=min(a,b)-vec2<f32>(1e-6))&&all(p<=max(a,b)+vec2<f32>(1e-6));
}
fn fluidSegmentsIntersect(a:vec2<f32>,b:vec2<f32>,c:vec2<f32>,d:vec2<f32>)->bool{
    let ab=b-a;let cd=d-c;let c0=fluidCross2(ab,c-a);let c1=fluidCross2(ab,d-a);
    let a0=fluidCross2(cd,a-c);let a1=fluidCross2(cd,b-c);
    if(((c0>0.0&&c1<0.0)||(c0<0.0&&c1>0.0))&&((a0>0.0&&a1<0.0)||(a0<0.0&&a1>0.0))){return true;}
    return fluidOnSegment(a,b,c)||fluidOnSegment(a,b,d)||fluidOnSegment(c,d,a)||fluidOnSegment(c,d,b);
}
fn fluidPolygonIntersectsRect(s:FluidShapeData,lo:vec2<f32>,hi:vec2<f32>)->bool{
    let start=s.polygon.z;let count=s.polygon.w;if(count<3u){return false;}
    let corners=array<vec2<f32>,4>(lo,vec2<f32>(hi.x,lo.y),hi,vec2<f32>(lo.x,hi.y));
    for(var i=0u;i<4u;i=i+1u){if(fluidInsidePolygon(s,corners[i])){return true;}}
    for(var i=0u;i<count;i=i+1u){
        let a=flow.points[start+i].xy;let b=flow.points[start+((i+1u)%count)].xy;
        if(all(a>=lo)&&all(a<=hi)){return true;}
        for(var edge=0u;edge<4u;edge=edge+1u){if(fluidSegmentsIntersect(a,b,corners[edge],corners[(edge+1u)%4u])){return true;}}
    }
    return false;
}
fn fluidShapeIntersectsWorldAabb(s:FluidShapeData,worldLo:vec3<f32>,worldHi:vec3<f32>)->bool{
    var lo=vec3<f32>(1e20);var hi=vec3<f32>(-1e20);
    for(var corner=0u;corner<8u;corner=corner+1u){
        let world=vec3<f32>(
            select(worldLo.x,worldHi.x,(corner&1u)!=0u),
            select(worldLo.y,worldHi.y,(corner&2u)!=0u),
            select(worldLo.z,worldHi.z,(corner&4u)!=0u));
        let local=fluidToLocal(s,world);lo=min(lo,local);hi=max(hi,local);
    }
    let kind=u32(s.scaleKind.w);
    if(kind==0u){let h=0.5*abs(s.params0.xyz);return all(hi>=-h)&&all(lo<=h);}
    if(kind==1u){
        let q=clamp(vec3<f32>(0.0),lo,hi);let r=abs(s.params0.x);return dot(q,q)<=r*r;
    }
    if(kind==2u||kind==3u){
        let halfHeight=0.5*abs(select(s.params0.y,s.params0.z,kind==3u));
        if(hi.y < -halfHeight || lo.y > halfHeight){return false;}
        let radius=select(abs(s.params0.x),max(abs(s.params0.x),abs(s.params0.y)),kind==3u);
        let nearest=clamp(vec2<f32>(0.0),lo.xz,hi.xz);
        if(dot(nearest,nearest)>radius*radius){return false;}
        if(kind==2u){
            let inner=min(radius,abs(s.params0.z));let farthest=max(abs(lo.xz),abs(hi.xz));
            if(dot(farthest,farthest)<inner*inner){return false;}
        }
        return true;
    }
    if(kind==4u){
        let r=abs(s.params0.x);let segment=max(0.0,0.5*abs(s.params0.y)-r);
        let nearest=clamp(vec2<f32>(0.0),lo.xz,hi.xz);
        let dy=max(0.0,max(lo.y-segment,-segment-hi.y));
        return dot(nearest,nearest)+dy*dy<=r*r;
    }
    let halfThickness=0.5*abs(s.params0.x);
    return hi.y>=-halfThickness&&lo.y<=halfThickness&&fluidPolygonIntersectsRect(s,lo.xz,hi.xz);
}`;
...
fn fluidReleaseCounter(counterIndex:u32){
    loop{let old=atomicLoad(&flowCounters[counterIndex]);if(old==0u){return;}if(atomicCompareExchangeWeak(&flowCounters[counterIndex],old,old-1u).exchanged){return;}}
}
...
fn fluidReleaseSink(index:u32){fluidReleaseCounter(${FLOW_EMITTER_COUNTER_U32}u+index);}
fn fluidEmitterRouteAvailable(mask:u32)->bool{
    for(var i=0u;i<${MAX_FLUID_EMITTERS}u;i=i+1u){
        if((mask&(1u<<i))==0u||flow.emitters[i].flags.x==0u){continue;}
        let counterIndex=i*2u;let budget=atomicLoad(&flowCounters[counterIndex+1u]);
        if(budget==0xffffffffu||atomicLoad(&flowCounters[counterIndex])<budget){return true;}
    }
    return false;
}
...
fn fluidSinkMatches(sink:FluidSinkData,world:vec3<f32>,particleIndex:u32,si:u32)->bool{
    ...
    if(perParticle&&!(fluidRnd(seed)<clamp(bitcast<f32>(sink.compat.z)*bitcast<f32>(sink.compat.w),0.0,1.0))){return false;}
    return true;
}
...
fn fluidTryDelete(world:vec3<f32>,particleIndex:u32)->bool{
    for(var si=0u;si<flow.header.y;si=si+1u){
        ...
        if(sink.compat.x==${FLOW_SINK_OPERATION_DELETE}u&&fluidSinkMatches(sink,world,particleIndex,si)&&fluidClaimSink(sink.route.w,sink.route.z)){return true;}
    }
    return false;
}
fn fluidTryRelaunch(world:vec3<f32>,particleIndex:u32)->FluidLaunch{
    ...
    for(var si=0u;si<flow.header.y;si=si+1u){
        let sink=flow.sinks[si];
        if(sink.compat.x!=${FLOW_SINK_OPERATION_RECYCLE}u||sink.route.y==0u||!fluidSinkMatches(sink,world,particleIndex,si)){continue;}
        if(!fluidEmitterRouteAvailable(sink.route.y)||!fluidClaimSink(sink.route.w,sink.route.z)){continue;}
        let seed=(flow.header.w*2654435761u)^(particleIndex*2246822519u)^(si*3266489917u);
        let ei=fluidChooseEmitter(sink.route.y,seed);if(ei>=flow.header.x){fluidReleaseSink(sink.route.w);continue;}let e=flow.emitters[ei];
