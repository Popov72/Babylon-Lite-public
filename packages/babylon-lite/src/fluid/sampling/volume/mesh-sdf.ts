...
export function buildMeshDistance(positions: Float32Array, indices: Uint32Array | Uint16Array): MeshDistance {
    ...
    const fillAggregates = (node: number): void => {
        ...
        const rL = Math.sqrt(dLx * dLx + dLy * dLy + dLz * dLz) + nodeR[left]!;
        const dRx = nodeC[3 * right]! - cX;
        const dRy = nodeC[3 * right + 1]! - cY;
        const dRz = nodeC[3 * right + 2]! - cZ;
        const rR = Math.sqrt(dRx * dRx + dRy * dRy + dRz * dRz) + nodeR[right]!;
    ...
    const isClosed = totalArea > 0 && Math.sqrt(vecAreaX * vecAreaX + vecAreaY * vecAreaY + vecAreaZ * vecAreaZ) / totalArea < OPEN_TOLERANCE;
