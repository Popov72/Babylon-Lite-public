...
import type { FluidPolygonSurface, FluidProfiler, FluidSim } from "./sim-common.js";
...
export function createFluidPolygonSurfaceTask(engine: EngineContext, scene: SceneContext, options: FluidPolygonSurfaceOptions): FluidPolygonSurfaceTask {
    ...
    let sims: readonly FluidSim[] = [options.sim];
    let enabled = false;
    let opacity = 1;
    ...
    function updateUniforms(): void {
        ...
        uniformData.set(inverseView, 32);
    ...
    function getSurfaceBinding(surface: FluidPolygonSurface): SurfaceBinding {
        ...
        if (!binding) {
            ...
            const maxDistance =
                Math.hypot(surface.gridDimensions[0] * surface.gridSpacing, surface.gridDimensions[1] * surface.gridSpacing, surface.gridDimensions[2] * surface.gridSpacing) +
                surface.gridSpacing;
