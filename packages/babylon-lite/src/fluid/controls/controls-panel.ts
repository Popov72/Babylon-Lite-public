...
//   • New hosts receive one normalized snapshot through `onApply`. Legacy `on.*` callbacks remain
//     immediate for standalone changes but are never replayed from a transaction.
...
import type { DiffuseParticleCounts, FluidPressureDiagnostics } from "../core/sim-common.js";
import { resolveFluidControlsCapabilities, type FluidControlsCapabilities } from "./controls-capabilities.js";
import {
...
export interface FluidFoamValues {
    ...
    debugTexture: string;
    /** Debug visualization that colors diffuse particles by spray/foam/bubble kind. */
    debugByKind?: boolean;
    softness: number;
    density: number;
...
export interface FluidControlsOptions {
    ...
    /** Legacy fine-grained host callbacks. New consumers should prefer `onApply`. */
    on?: FluidControlsCallbacks;
    /** Atomic normalized application boundary. One logical setter or successful transaction invokes
     *  this at most once with the complete current snapshot and coalesced top-level keys. The
     *  callback must validate before mutating: synchronous JavaScript cannot roll back mutations
     *  already made inside a callback that later throws. */
    onApply?(values: FluidControlValues, changedKeys: readonly (keyof FluidControlValues)[]): void;
    /** Features implemented by the embedding host. These are intersected with backend/device
     *  capabilities, and unsupported controls are hidden rather than rendered as no-ops. */
    capabilities?: Partial<FluidControlsCapabilities>;
...
export interface FluidControlsHandle {
    ...
    /** @internal */
    readonly makeSection: (title: string, items: HTMLElement[]) => HTMLElement[];
    ...
    /** @internal */
    readonly setMaterial: (material: number) => void;
    /** @internal */
    readonly setParticleCount: (count: number) => void;
    /** @internal */
    readonly setParticleCountVisible: (visible: boolean) => void;
    /** @internal */
    readonly setActiveParticleCount: (count: number) => void;
    /** @internal */
    readonly setParticleUsage: (
        activeCount: number,
        totalCount: number,
        gpuBytes: number,
    ...
    /** @internal */
    readonly setPressureDiagnostics: (diagnostics: FluidPressureDiagnostics | undefined) => void;
    /** @internal */
    readonly setSimulationDuration: (seconds: number) => void;
    /** @internal */
    readonly setAlphaDecay: (seconds: number) => void;
    /** @internal */
    readonly setRenderMode: (spheres: boolean) => void;
    /** @internal */
    readonly setPolygonShader: (mode: "physical" | "ocean") => void;
    /** @internal */
    readonly setColor: (hex: string) => void;
    /** @internal */
    readonly setIndependentRendering: (enabled: boolean) => void;
    /** @internal */
    readonly setAbsorption: (v: number) => void;
    /** @internal */
    readonly setParticleSize: (v: number) => void;
    /** @internal */
    readonly setRefraction: (v: number) => void;
    /** @internal */
    readonly setSpecular: (v: number) => void;
    /** @internal */
    readonly setReflection: (exposure: number, contrast: number) => void;
    /** @internal */
    readonly setReflectivity: (v: number) => void;
    /** @internal */
    readonly setDepthBlur: (size: number, threshold: number) => void;
    /** @internal */
    readonly setThicknessBlur: (v: number) => void;
    /** @internal */
    readonly setHalf: (on: boolean) => void;
    /** @internal */
    readonly setSurfaceFilter: (m: "bilateral" | "narrowRange") => void;
    /** @internal */
    readonly setNarrowRange: (delta: number, mu: number) => void;
    /** @internal */
    readonly setAnisotropic: (on: boolean) => void;
    /** @internal */
    readonly setAnisotropySurfScale: (v: number) => void;
    /** @internal */
    readonly setThicknessDownscale: (v: number) => void;
    /** @internal */
    readonly setShowContainer: (on: boolean) => void;
    /** @internal */
    readonly setDebug: (mode: string) => void;
    /** @internal */
    readonly setPhysics: (schema: Record<string, number>) => void;
    /** Run `apply` inside a controls transaction. A successful outer transaction emits one
     *  normalized `onApply` snapshot; a throwing body emits nothing. */
    /** @internal */
    readonly runTransaction: (apply: () => void) => void;
    /** @internal */
    readonly setPhysScale: (scale: number) => void;
    /** @internal */
    readonly setPhysScaleVisible: (visible: boolean) => void;
    /** @internal */
    readonly setGridResolution: (resolution: number) => void;
    /** @internal */
    readonly setMarkersPerCell: (markersPerCell: number) => void;
    /** @internal */
    readonly setFlipParticleCapacity: (capacity: number) => void;
    /** @internal */
    readonly setMarkerDensityWarning: (message: string) => void;
    /** @internal */
    readonly setGridSettings: (position: [number, number, number], size: [number, number, number], cellSize: number) => void;
    /** @internal */
    readonly setGridStatus: (message: string) => void;
    /** @internal */
    readonly setShowGridBounds: (visible: boolean) => void;
    /** @internal */
    readonly setShowGridBoundsSolid: (visible: boolean) => void;
    /** @internal */
    readonly setActiveBlocks: (enabled: boolean) => void;
    /** @internal */
    readonly setPagedGrid: (enabled: boolean) => void;
    /** @internal */
    readonly setPagedGridMaxPages: (pages: number, limit?: number) => void;
    /** @internal */
    readonly setPagedGridStatus: (message: string, error?: boolean) => void;
    /** @internal */
    readonly setFusedBlockDiscovery: (enabled: boolean) => void;
    /** @internal */
    readonly setFoam: (foam: FluidFoamValues) => void;
    /** @internal */
    readonly setFoamParticleCounts: (counts: DiffuseParticleCounts | undefined, enabled: boolean, capacity?: number) => void;

    /** Snapshot every control value (for pair-state capture / export). */
    /** @internal */
    readonly getValues: () => FluidControlValues;
    /** Physics-slider values for a given method (source of truth for applying to the sim). */
    /** @internal */
    readonly getPhysicsValues: (method: string) => Record<string, number>;
    /** Rebuild the physics-slider block for a method (called on method change). */
    /** @internal */
    readonly rebuildPhysics: (method: string) => void;
    ...
    /** @internal */
    readonly setVisiblePhysicsParams: (keys: string[] | null) => void;

    /** GPU-timing panel handle, or null when hidden / not configured. */
    gpu: FluidGpuHandle | null;
}

export function getFluidControlValues(controls: FluidControlsHandle): FluidControlValues {
    return controls.getValues();
}

export function getFluidControlPhysicsValues(controls: FluidControlsHandle, method: string): Record<string, number> {
    return controls.getPhysicsValues(method);
}

export function runFluidControlsHandleTransaction(controls: FluidControlsHandle, apply: () => void): void {
    controls.runTransaction(apply);
}

export function refreshFluidGpuMemory(gpu: FluidGpuHandle, simulationBytes: number, canvasWidth: number, canvasHeight: number): void {
...
const CALLBACK_CHANGED_KEYS: Partial<Record<keyof FluidControlsCallbacks, keyof FluidControlValues | readonly (keyof FluidControlValues)[]>> = {
    onMethod: "method",
    onMaterial: "material",
    onParticleCount: "count",
    onSimulationDuration: "simulationDuration",
    onAlphaDecay: "alphaDecay",
    onRenderMode: "renderMode",
    onPolygonShader: "polygonShader",
    onColor: "color",
    onIndependentRendering: "independentRendering",
    onAbsorption: "absorption",
    onParticleSize: "size",
    onRefraction: "refraction",
    onSpecular: "specular",
    onReflection: ["reflectionExposure", "reflectionContrast"],
    onReflectivity: "reflectivity",
    onDepthBlur: ["depthBlur", "depthBlurThreshold"],
    onThicknessBlur: "thicknessBlur",
    onHalf: "half",
    onSurfaceFilter: "surfaceFilter",
    onNarrowRange: ["narrowDelta", "narrowMu"],
    onAnisotropic: "anisotropic",
    onAnisotropySurfScale: "anisoSurfScale",
    onThicknessDownscale: "thicknessDownscale",
    onShowContainer: "showContainer",
    onDebug: "debug",
    onPhysicsParam: "schema",
    onPolygonSurface: "schema",
    onPhysScale: "physScale",
    onGridResolution: "gridResolution",
    onMarkersPerCell: "markersPerCell",
    onFlipParticleCapacity: "count",
    onShowGridBounds: "showGridBounds",
    onShowGridBoundsSolid: "showGridBoundsSolid",
    onActiveBlocks: "activeBlocks",
    onPagedGrid: "pagedGrid",
    onPagedGridMaxPages: "pagedGridMaxPages",
    onFusedBlockDiscovery: "fusedBlockDiscovery",
    onFoamEnable: "foam",
    onFoamActiveParticles: "foam",
    onFoamKinds: "foam",
    onFoamSurfaceFiltering: "foam",
    onFoamKta: "foam",
    onFoamKwc: "foam",
    onFoamAdvanced: "foam",
    onFoamLifetime: "foam",
    ...
    onFoamAO: "foam",
    onFoamNormal: "foam",
    onFoamDebugByKind: "foam",
    onFoamDebugTexture: "foam",
};
...
export function createFluidControlsPanel(opts: FluidControlsOptions): FluidControlsHandle {
    ...
    const controlsTransaction = createFluidControlsTransaction<FluidControlValues, keyof FluidControlValues>({
        read: snapshotControlValues,
        onApply: opts.onApply,
        restore: restoreControlValues,
    });
    const transactionalOn = wrapFluidControlsCallbacks(opts.on ?? {}, controlsTransaction, {
        immediate: ["onGridSettings"],
        changedKeys: CALLBACK_CHANGED_KEYS,
    });
    const on = new Proxy(transactionalOn, {
        get(target, property, receiver) {
            const value = Reflect.get(target, property, receiver) as unknown;
            return restoringControls && typeof value === "function" ? () => undefined : value;
        },
    });
    ...
    const applyMaterialVisibility = (): void => {
        materialRow.style.display = methodCaps().material ? "block" : "none";
    };
    methodSel.onchange = () => {
    ...
    particleGpuMemoryValue.dataset.fluidSimulationGpuMemory = "true";
    ...
    const updatePressureDiagnostics = (): void => {
        ...
        pressureDiagnosticsRow.style.display = methodCaps().pressureDiagnostics && diagnostics ? "block" : "none";
        if (!diagnostics) {
    ...
    const applySurfaceVisibility = (spheres: boolean): void => {
        ...
        setRowVisible(independentRenderingRow, !spheres && methodCaps().independentRendering);
    ...
    independentRenderingChk.checked = methodCaps().independentRendering && (init.independentRendering ?? false);
    ...
    const commitGridSettings = (): void => {
        ...
        if (typeof error !== "string" && controlsTransaction.appliesSnapshots) {
            markFluidControlsChanged(controlsTransaction, "gridPosition", "gridSize");
        }
    };
    ...
    const syncPagedGridCapacityInput = (): void => {
        ...
        pagedGridCapacityInput.value = String(
            pagedGridCapacityPages >= pagedGridCapacityLimit ? units : Math.max(1, Math.min(units, Math.round(pagedGridCapacityPages / pagedGridCapacityStep)))
        );
    };
    ...
    const applyActiveBlocksVisibility = (): void => {
        const mpmDisplay = showActiveBlocks && methodCaps().activeBlocks ? "flex" : "none";
        const pagedDisplay = showActiveBlocks && methodCaps().pagedGrid ? "flex" : "none";
    ...
    if (!opts.hideGpuTiming && opts.gpu && methodCaps().timing) {
    ...
    function restoreControlValues(values: FluidControlValues): void {
        restoringControls = true;
        try {
            handle.setMethod(values.method);
            handle.setMaterial(values.material);
            handle.setPhysics(values.schema);
            handle.rebuildPhysics(values.method);
            handle.setSimulationDuration(values.simulationDuration);
            handle.setAlphaDecay(values.alphaDecay);
            handle.setColor(values.color);
            handle.setIndependentRendering(values.independentRendering);
            handle.setHalf(values.half);
            handle.setThicknessDownscale(values.thicknessDownscale);
            handle.setAbsorption(values.absorption);
            handle.setParticleSize(values.size);
            handle.setPhysScale(values.physScale);
            handle.setGridSettings(values.gridPosition, values.gridSize, values.cellSize);
            handle.setGridResolution(values.gridResolution);
            handle.setMarkersPerCell(values.markersPerCell);
            if (values.method === "FLIP") {
                handle.setFlipParticleCapacity(values.count);
            } else {
                handle.setParticleCount(values.count);
            }
            handle.setRenderMode(values.renderMode === "spheres");
            handle.setPolygonShader(values.polygonShader);
            handle.setRefraction(values.refraction);
            handle.setSpecular(values.specular);
            handle.setReflection(values.reflectionExposure, values.reflectionContrast);
            handle.setReflectivity(values.reflectivity);
            handle.setDepthBlur(values.depthBlur, values.depthBlurThreshold);
            handle.setThicknessBlur(values.thicknessBlur);
            handle.setSurfaceFilter(values.surfaceFilter);
            handle.setNarrowRange(values.narrowDelta, values.narrowMu);
            handle.setAnisotropic(values.anisotropic);
            handle.setAnisotropySurfScale(values.anisoSurfScale);
            handle.setShowGridBounds(values.showGridBounds);
            handle.setShowGridBoundsSolid(values.showGridBoundsSolid);
            handle.setActiveBlocks(values.activeBlocks);
            handle.setPagedGrid(values.pagedGrid);
            handle.setPagedGridMaxPages(values.pagedGridMaxPages);
            handle.setFusedBlockDiscovery(values.fusedBlockDiscovery);
            handle.setDebug(values.debug);
            handle.setShowContainer(values.showContainer);
            handle.setFoam(values.foam);
        } finally {
            restoringControls = false;
        }
    }

    function snapshotControlValues(): FluidControlValues {
        ...
        return {
            ...
            independentRendering: methodCaps().independentRendering && independentRenderingChk.checked,
            ...
            foam: {
                ...
                debugByKind: foamDebugChk.checked,
                softness: foamT0,
                density: foamT1,
    ...
    const handle: FluidControlsHandle = {
        ...
        setIndependentRendering(enabled: boolean): void {
            independentRenderingChk.checked = methodCaps().independentRendering && enabled;
            if (methodCaps().independentRendering) {
        ...
        runTransaction(apply: () => void): void {
            runFluidControlsTransaction(controlsTransaction, apply);
        },
        ...
        setPagedGridStatus(message: string, error = false): void {
            ...
            pagedGridStatus.style.display = message && showActiveBlocks && methodCaps().pagedGrid ? "block" : "none";
        },
        setFusedBlockDiscovery(enabled: boolean): void {
            fusedBlockDiscoveryChk.checked = enabled;
        },
        setFoam(foam: FluidFoamValues): void {
            runFluidControlsTransaction(controlsTransaction, () => {
                ...
                applyFoamKindAvailability();
                foamTMin = foam.tMin;
                foamDebugChk.checked = foam.debugByKind ?? false;
                ...
                // Density first so the t0 <= t1 clamp restores the exact pair.
        ...
        getValues: snapshotControlValues,
