...
const ANAGLYPH_EXTRA_TEXTURE_WGSL = wgsl`@group(0) @binding(2) var leftTextureSampler:texture_2d<f32>;`;

const ANAGLYPH_FRAGMENT_WGSL = wgsl`fn applyPostProcess(color:vec4f, uv:vec2f)->vec4f{let l=textureSampleLevel(leftTextureSampler,sourceSampler,clamp(uv,vec2f(0),vec2f(1)),0);let left=vec4f(1,l.g,l.b,1);let right=vec4f(color.r,1,1,1);return vec4f(right.rgb*left.rgb,1);}`;
