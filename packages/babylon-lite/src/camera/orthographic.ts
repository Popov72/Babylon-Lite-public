...
import { writeOrthoOffCenterMat4LHIntoBuffer } from "../math/write-ortho-off-center-mat4-lh-into-buffer.js";
...
function writeOrthoProjection(camera: Camera, aspectRatio: number, out: Mat4Storage): void {
    ...
    writeOrthoOffCenterMat4LHIntoBuffer(out, b.left ?? -halfWidth, b.right ?? halfWidth, b.bottom ?? -halfHeight, b.top ?? halfHeight, camera.nearPlane, camera.farPlane);
}
...
export function disableOrthographicCamera(camera: Camera): void {
    camera.ortho = null;
    // `writePerspectiveMat4LHIntoBuffer` only writes the terms a perspective matrix needs, relying on the
