...
function emitBlend(
    ...
    stageState.body.push(wgsl`let ${out}_stepR = step(0.5, (${n0}).r);`);
    stageState.body.push(wgsl`let ${out}_stepG = step(0.5, (${n0}).g);`);
    stageState.body.push(
        wgsl`let ${out} = vec3<f32>((1.0 - ${out}_stepR) * (${n0}).r * (${n1}).r * 2.0 + ${out}_stepR * (1.0 - (1.0 - (${n0}).r) * (1.0 - (${n1}).r) * 2.0), (1.0 - ${out}_stepG) * (${n0}).g * (${n1}).g * 2.0 + ${out}_stepG * (1.0 - (1.0 - (${n0}).g) * (1.0 - (${n1}).g) * 2.0), (${n0}).b * (${n1}).b);`
    );
