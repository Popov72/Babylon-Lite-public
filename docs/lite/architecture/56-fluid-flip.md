...
Paged-grid enablement and page-capacity edits are staged in the controls and applied together on simulation reset. Resizing still requires new page-backed buffers and bind groups because WebGPU buffer sizes and bound resources are immutable. Page capacity is supplied to WGSL through runtime uniforms rather than shader constants, and compute pipelines are cached per device, so capacity-only rebuilds do not recompile the solver. Dense and paged storage remain separate shader variants because their indexing and bind-group layouts differ.
...
Active-list foam rendering reads the authoritative ping-pong side directly from the GPU state buffer. If overflow gates prepare/update/finalize dispatches, the side and draw count therefore remain paired with the last completed list. The renderer never speculatively follows the CPU-side frame toggle, which previously made many instances read unwritten zero indices and splat diffuse slot zero into a bright lump.
...
A face touching a solid cell uses the installed scene SDF at the current pose. When moving-solid boundaries are enabled, FLIP samples the SDF at a small positive time offset to derive the boundary's normal velocity. Format-15 Blender bundles drive that time-varying SDF from current and previous glTF node matrices, so animated rigid obstacles displace liquid instead of behaving like teleported static walls.
