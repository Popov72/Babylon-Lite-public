...
- **Opt-in features pay for themselves at their enabler.** A feature that must be explicitly enabled (device-lost recovery, capture hooks, diagnostics) owns *all* of its code behind the enable function. Modules that merely *feed* the feature must contain no feature semantics — no metadata encoding, no branching on feature state, no imports of feature modules. The only acceptable footprint in a general-purpose module is a single optional-chained call on an engine-installed seam (e.g. `engine._dlr?.x(...)`), where the seam object exists only after the feature is enabled. All meaning lives in the seam implementation.
- **Never pass a branch-gating object to an optional seam — pass the already-computed scalar.** This is about the *runtime value*, not the type annotation (types are erased and cost nothing). Rollup tracks the property values of object literals and uses them to prove branches dead. Handing such an object to an unknown callee (`engine._dlr?.x(bgOptions)`) forces Rollup to **deoptimize** it — the callee might mutate it — so it loses the known property values and keeps branches it would otherwise have eliminated, including the `await import()` inside them. Measured in `load-env.ts`: passing `bgOptions` (whose `skipSkybox`/`skipGround` gate dynamic background imports) retained `background-ground.js` (+4,968 B) and `background-solid-skybox.js` (+1,882 B) in 55 scenes that had tree-shaken them; passing the pre-computed boolean cost 31 B. Passing an *unrelated* object is harmless (measured: +27 B) — the hazard is specifically an object whose properties gate tree-shakeable code.
- **Bundle-size regressions in unrelated scenes are a design defect, not a new baseline.** If a change grows scenes that don't use the feature, fix the architecture. Never absorb the growth by moving the published baseline, and never raise a `maxRawKB` ceiling without explicit user approval.
...
- **Never write inline `typeof import("./mod.js")` type annotations — always use a top-level `import type`.** To type a lazily-imported module or one of its exports, add `import type * as Mod from "./mod.js"` or `import type { fn } from "./mod.js"` at the top of the file and annotate with `Mod` / `typeof fn`. `import type` is fully erased, so this costs nothing at runtime and does not create a static edge for the bundler — but unlike the inline form it keeps the module path in one place, stays greppable/refactorable by tooling, and reads like every other import.
