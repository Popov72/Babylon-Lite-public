...
- **Bundle-size regressions in unrelated scenes are a design defect, not a new baseline.** If a change grows scenes that don't use the feature, fix the architecture. Never absorb the growth by regenerating manifests, and never raise a `maxRawKB` ceiling without explicit user approval.
