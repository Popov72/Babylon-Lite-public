...
try {
    ...
    const result = await page.evaluate(async () => {
        ...
        const nestedField = (host, label) =>
            [...host.querySelectorAll(".behavior-nested")].find((row) => row.querySelector(":scope > .behavior-label")?.textContent.replace(" *", "") === label);
    ...
    if (
        !result.fluidActionOffers.includes("pauseSimulation") ||
        !result.fluidActionOffers.includes("unpauseSimulation") ||
        !result.fluidActionOffers.includes("enablePlayerCollision") ||
        !result.fluidActionOffers.includes("disablePlayerCollision")
    ) {
        fail(`fluid lifecycle actions were not offered: ${JSON.stringify(result.fluidActionOffers)}`);
    }
    const fluidActionsAt = result.fluidPropertyOrder.indexOf("eventActions");
    const fluidDurationAt = result.fluidPropertyOrder.indexOf("shutdownDuration");
    const fluidDecayAt = result.fluidPropertyOrder.indexOf("shutdownAlphaDecay");
    if (fluidActionsAt < 0 || fluidDurationAt < 0 || fluidDecayAt < 0 || fluidDurationAt > fluidActionsAt || fluidDecayAt > fluidActionsAt) {
        fail(`fluid shutdown fields were buried below event actions: ${JSON.stringify(result.fluidPropertyOrder)}`);
    }
    if (result.chunkSource.value !== "CH01" || result.chunkSource.label !== "CH01 chunk" || !same(result.chunkSource.events, ["notVisible", "visible"])) {
    ...
    if (
        ...
        result.fluid.shutdownDuration !== 6 ||
        result.fluid.shutdownAlphaDecay !== 1.5 ||
    ...
    if (result.serializedPresets?.stdLiquefaction?.base !== "liquefaction" || "liquefiable" in result.serializedPresets.stdLiquefaction || result.serializedHasLegacyLibrary) {
