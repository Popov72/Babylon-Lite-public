...
/** Per-particle UVs plus, when per-vertex texture indices are supplied, the texture each particle
 *  samples (`0xffffffff` when the particle matched no triangle). */
export interface ParticleUvResult {
    uvs: Float32Array;
 ...
 *
 * @param particles xyz per particle, in the SAME space as `verts`.
 * @param count number of particles to process.
 * @param verts xyz per mesh vertex.
 * @param indices triangle list into `verts`.
 * @param vertUvs uv per mesh vertex (aligned with `verts`).
 * @param vertTex optional per-vertex texture index (a triangle lies in one sub-mesh, so its first
 *        vertex decides); pass null when the mesh has a single texture.
 */
export function computeParticleUvs(
    ...
    if (nTri === 0 || verts.length === 0) return { uvs, texIndices };
    ...
    for (let p = 0; p < count; p++) {
        ...
        if (vertTex) texIndices[p] = vertTex[ia]!;
    }
...
function buildTriangleBvh(verts: Float32Array, indices: Uint32Array, nTri: number): TriBvh {
    ...
    for (let t = 0; t < nTri; t++) order[t] = t;
    ...
    const fitBounds = (node: number): void => {
        ...
        for (let i = s; i < e; i++) {
            const t = order[i]! * 3;
            if (triMin[t]! < x0) x0 = triMin[t]!;
            if (triMin[t + 1]! < y0) y0 = triMin[t + 1]!;
            if (triMin[t + 2]! < z0) z0 = triMin[t + 2]!;
            if (triMax[t]! > x1) x1 = triMax[t]!;
            if (triMax[t + 1]! > y1) y1 = triMax[t + 1]!;
            if (triMax[t + 2]! > z1) z1 = triMax[t + 2]!;
        }
        nodeMin[node * 3] = x0;
        nodeMin[node * 3 + 1] = y0;
        nodeMin[node * 3 + 2] = z0;
    ...
    while (todo.length) {
        ...
        let mean = 0;
        for (let i = s; i < s + n; i++) mean += centroid[order[i]! * 3 + axis]!;
        mean /= n;
        let i = s;
        let j = s + n - 1;
        while (i <= j) {
...
function closestTriangle(
    ...
    while (sp > 0) {
        const node = stack[--sp]!;
        if (aabbDist2(nodeMin, nodeMax, node, px, py, pz) >= bestD2) continue;
        const n = nodeCount[node]!;
        if (n === 0) {
