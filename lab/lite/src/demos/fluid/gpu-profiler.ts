// Lab-only GPU profiler for the fluid demo (opt-in "Timing" section).
//
// This is the CONCRETE implementation of the package's type-only `FluidProfiler`
// hook (packages/babylon-lite/src/fluid/sim-common.ts). ALL of the timestamp-query
// machinery lives here in the lab so the babylon-lite package pays zero bytes for
// it unless an app opts in: the sims / render tasks only reference the `FluidProfiler`
// TYPE (erased at compile time) and add `timestampWrites: profiler?.pass(stage)` to
// their pass descriptors, which is a no-op while no profiler is installed.
//
...
import type { FluidProfiler } from "babylon-lite/fluid/sim-common.js";
