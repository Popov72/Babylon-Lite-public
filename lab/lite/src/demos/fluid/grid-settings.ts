...
export const gridLocalToWorld = (localPosition: readonly [number, number, number], gridPosition: readonly [number, number, number]): [number, number, number] => [
    localPosition[0] + gridPosition[0],
    localPosition[1] + gridPosition[1],
    localPosition[2] + gridPosition[2],
];
export const worldToGridLocal = (worldPosition: readonly [number, number, number], gridPosition: readonly [number, number, number]): [number, number, number] => [
    worldPosition[0] - gridPosition[0],
    worldPosition[1] - gridPosition[1],
    worldPosition[2] - gridPosition[2],
];
...
export const scaledGridSize = (size: readonly [number, number, number], scale: readonly [number, number, number], increment = 0.1): [number, number, number] =>
    size.map((value, index) => {
        const steps = Math.max(1, Math.round((value * Math.abs(scale[index]!)) / increment));
        return Number((steps * increment).toFixed(6));
    }) as [number, number, number];
export const gridSettingsFromGizmoTransform = (
    currentSize: readonly [number, number, number],
    position: readonly [number, number, number],
    scale: readonly [number, number, number]
): { position: [number, number, number]; size: [number, number, number] } => ({
    position: [...position],
    size: scaledGridSize(currentSize, scale),
});
