// Babylon.js reference for Scene 271 — renders the FINAL configuration of the Lite scene
// (spot light B + its PCF shadow generator). The Lite side reaches this state by removing
// light A and rebuilding, so a correct rebuild must match this image.
...
(async function () {
    ...
    const ground01 = MeshBuilder.CreateGround("Shadow Light Rebuild", { width: 24, height: 60 }, scene);

    const groundMaterial = new StandardMaterial("ground", scene);
    groundMaterial.diffuseColor = new Color3(0.75, 0.72, 0.66);
    ...
    // Light B — the post-rebuild configuration.
    const light00 = new SpotLight("*spot00", new Vector3(8, 20, 0), new Vector3(-0.3, -1, 0), 1.2, 24, scene);
