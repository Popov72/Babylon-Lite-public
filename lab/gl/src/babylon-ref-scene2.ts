...
/**
 * Babylon.js reference for GL Scene 2 — Plasma.
 *
 * Renders the SAME procedural plasma as lab/gl/src/scene2.ts (which uses
 * @babylonjs/lite-gl's `runFullscreenEffect`) but via Babylon's ThinEngine +
 * EffectRenderer + EffectWrapper, so the parity harness can diff the two
 * pixel-for-pixel.
 *
 ...
 *  - Fragment: the SAME expression as scene2, written in ES1.00 form
 *    (varying / gl_FragColor). Babylon's WebGL2 shader processor auto-converts it
 *    to ES3.00 — the target lite-gl authors directly. uResolution drives the same
 *    aspect correction so the plasma's circular ripple is identical.
